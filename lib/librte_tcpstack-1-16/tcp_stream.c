...
/*---------------------------------------------------------------------------*/
inline void
InitializeTCPStreamManager(void)
{
    next_seed = time(NULL);
}
/*---------------------------------------------------------------------------*/
...
RaiseReadEvent(mtcp_manager_t mtcp, tcp_stream *stream)
{
    if (stream->socket) {
        ...
        TRACE_DEBUG("Stream %d: Raising read without a socket!\n", stream->id);
    }
}
/*---------------------------------------------------------------------------*/
inline void 
RaiseWriteEvent(mtcp_manager_t mtcp, tcp_stream *stream)
{
    if (stream->socket) {
        ...
        TRACE_DEBUG("Stream %d: Raising write without a socket!\n", stream->id);
    }
}
/*---------------------------------------------------------------------------*/
inline void 
RaiseCloseEvent(mtcp_manager_t mtcp, tcp_stream *stream)
{
    if (stream->socket) {
        ...
        TRACE_DEBUG("Stream %d: Raising close without a socket!\n", stream->id);
    }
}
/*---------------------------------------------------------------------------*/
inline void 
RaiseErrorEvent(mtcp_manager_t mtcp, tcp_stream *stream)
{
    if (stream->socket) {
        ...
        TRACE_DEBUG("Stream %d: Raising error without a socket!\n", stream->id);
    }
}
/*---------------------------------------------------------------------------*/
        ...
        uint32_t saddr, uint16_t sport, uint32_t daddr, uint16_t dport)
{
    ...
    rte_spinlock_lock(&mtcp->ctx->flow_pool_lock);

    stream = (tcp_stream *)MPAllocateChunk(mtcp->flow_pool);
    if (!stream) {
        TRACE_DEBUG("Cannot allocate memory for the stream. CONFIG.max_concurrency: %d, concurrent: %u\n", CONFIG.max_concurrency, mtcp->flow_cnt);
        rte_spinlock_unlock(&mtcp->ctx->flow_pool_lock);
        return NULL;
    }
    ...
    if (ret < 0) {
        TRACE_DEBUG("Stream %d: Failed to insert the stream into hash table.\n", stream->id);
        MPFreeChunk(mtcp->flow_pool, stream);
        rte_spinlock_unlock(&mtcp->ctx->flow_pool_lock);
        return NULL;
    }
    ...
    if (ret < 0) {
        TRACE_DEBUG("Stream %d: Failed to insert the stream into SID lookup table.\n", stream->id);
        MPFreeChunk(mtcp->flow_pool, stream);
        rte_spinlock_unlock(&mtcp->ctx->flow_pool_lock);
        return NULL;
    }
    ...
    rte_spinlock_init(&stream->rcvvar->read_lock);
    rte_spinlock_init(&stream->sndvar->write_lock);

    sa = (uint8_t *)&stream->saddr;
    da = (uint8_t *)&stream->daddr;
    TRACE_DEBUG("CREATED NEW TCP STREAM %d: %u.%u.%u.%u(%d) -> %u.%u.%u.%u(%d) (ISS: %u)\n", stream->id, sa[0], sa[1], sa[2], sa[3], ntohs(stream->sport), da[0], da[1], da[2], da[3], ntohs(stream->dport), stream->sndvar->iss);
...
tcp_stream *
CreateTCPStream_by_port(int type, uint32_t init_seq, 
		uint32_t saddr, uint16_t sport, uint32_t daddr, uint16_t dport)
{
	tcp_stream *stream = NULL;
	...
	stream = (tcp_stream *)MPAllocateChunk(mtcp->flow_pool);
	if (!stream) {
		TRACE_DEBUG("Cannot allocate memory for the stream. "
...
/*
	...
	sprintf(pool_name, "host_buff_pool%d_%d", rte_lcore_id(), stream->id);
	//printf("%s-%d: pool_name:%s\n",__func__,__LINE__,pool_name);
	stream->sndvar->pre_read_list.poor = MPCreate(pool_name, sizeof(struct tcp_prepare_read),
									sizeof(struct tcp_prepare_read) * 512);
	 if (!stream->sndvar->pre_read_list.poor) {
			 printf("%s alloc fail\n", pool_name);
...
*/
	...
	ret = StreamHTInsert(mtcp->tcp_flow_table, stream);
	if (ret < 0) {
		TRACE_DEBUG("Stream %d: "
				"Failed to insert the stream into hash table.\n", stream->id);
		
		MPDestroy(stream->sndvar->pre_read_list.poor);
		MPFreeChunk(mtcp->sv_pool, stream->sndvar);
	 	MPFreeChunk(mtcp->rv_pool, stream->rcvvar);
		MPFreeChunk(mtcp->flow_pool, stream);
		//pthread_mutex_unlock(&mtcp->ctx->flow_pool_lock);
...
*/
	...
	stream->sndvar->wscale_peer = 0;
	//stream->sndvar->nif_out = GetOutputInterface(stream->daddr, &is_external);
	stream->sndvar->nif_out = -1;
	stream->is_external = is_external;

	stream->sndvar->iss = init_seq;
	//stream->sndvar->iss = 0;
	stream->rcvvar->irs = 0;
...
*/
	stream->sndvar->snd_wnd = CONFIG.sndbuf_size;
	stream->sndvar->tcp_data_ring.free_num = TCP_SEND_DATA_BUFFER_MAX_NUM;
	stream->rcv_nxt = 0;
	stream->rcvvar->rcv_wnd = TCP_INITIAL_WINDOW;
...
*/
	sa = (uint8_t *)&stream->saddr;
	da = (uint8_t *)&stream->daddr;
	TRACE_DEBUG("CREATED NEW TCP STREAM %d: "
...
DestroyTCPStream(mtcp_manager_t mtcp, tcp_stream *stream)
{
    ...
    sa = (uint8_t *)&stream->saddr;
    da = (uint8_t *)&stream->daddr;
    TRACE_DEBUG("DESTROY TCP STREAM %d: "
    ...
    if (stream->sndvar->rstat.tdp_ack_cnt) {
        TRACE_DEBUG("Stream %d: triple duplicated ack: %u, "
    ...
    if (stream->sndvar->rstat.sack_cnt) {
        TRACE_DEBUG("Selective ack count: %u, bytes: %u, "
                "average bytes/ack: %u\n", 
    ...
    if (stream->sndvar->rstat.tdp_sack_cnt) {
        TRACE_DEBUG("Selective tdp ack count: %u, bytes: %u, "
                "average bytes/ack: %u\n", 
    ...
    if (bound_addr) {
        ...
        if (ret < 0) {
            TRACE_DEBUG("(NEVER HAPPEN) Failed to free address.\n");
        }
    }
    ...
    TRACE_DEBUG("Destroyed. Remaining flows: %u\n", mtcp->flow_cnt);
...
void
TOE_DestroyTCPStream(tcp_stream *stream)
{
	struct sockaddr_in addr;
	int bound_addr = FALSE;
	uint8_t *sa, *da;
...
*/
	assert(stream->on_hash_table == TRUE);
	if (stream->sndvar->tcp_data_ring.una_head != stream->sndvar->tcp_data_ring.tail) {
		if (stream->sndvar->tcp_data_ring.una_head < stream->sndvar->tcp_data_ring.tail)
			rte_pktmbuf_free_bulk(&stream->sndvar->tcp_data_ring.m[stream->sndvar->tcp_data_ring.una_head], stream->sndvar->tcp_data_ring.tail - stream->sndvar->tcp_data_ring.una_head);
		else {	
			rte_pktmbuf_free_bulk(&stream->sndvar->tcp_data_ring.m[stream->sndvar->tcp_data_ring.una_head], TCP_SEND_DATA_BUFFER_MAX_NUM - stream->sndvar->tcp_data_ring.una_head);
			rte_pktmbuf_free_bulk(stream->sndvar->tcp_data_ring.m, stream->sndvar->tcp_data_ring.tail);
		}
	}
/*
	if (stream->sndvar->pre_read_list.poor)
...
*/
	...
	if (stream->rcvvar->rcvbuf) {
		//RBFree(mtcp->rbm_rcv, stream->rcvvar->rcvbuf);
		MPFreeChunk(mtcp->rvbuf_pool, stream->rcvvar->rcvbuf);
		stream->rcvvar->rcvbuf = NULL;
	}
...
DumpStream(mtcp_manager_t mtcp, tcp_stream *stream)
{
    ...
    sa = (uint8_t *)&stream->saddr;
    da = (uint8_t *)&stream->daddr;
    thread_printf("========== Stream %u: "
            "%u.%u.%u.%u(%u) -> %u.%u.%u.%u(%u) ==========\n", stream->id, 
            sa[0], sa[1], sa[2], sa[3], ntohs(stream->sport), 
            da[0], da[1], da[2], da[3], ntohs(stream->dport));
    thread_printf("Stream id: %u, type: %u, state: %s, close_reason: %s\n",  
    ...
    if (stream->socket) {
        ...
        thread_printf("Socket id: %d, type: %d, opts: %u\n"
    ...
    if (sndvar->sndbuf) {
        ...
        thread_printf("Send buffer: (null)\n");
    }
    thread_printf(
    ...
    if (rcvvar->rcvbuf) {
        ...
        thread_printf("Receive buffer: (null)\n");
    }
    thread_printf("last_ack_seq: %u, dup_acks: %u\n", 

...
{
    int i;
    struct rto_hashstore* hs = rte_zmalloc(NULL, sizeof(struct rto_hashstore), RTE_CACHE_LINE_SIZE);
    if (!hs) {
        TRACE_DEBUG("calloc: InitHashStore");
        return 0;
    }
...
AddtoRTOList(mtcp_manager_t mtcp, tcp_stream *cur_stream)
{
    ...
    if (cur_stream->on_rto_idx < 0 ) {
        if (cur_stream->on_timewait_list) {
            TRACE_DEBUG("Stream %u: cannot be in both rto and timewait list.\n", cur_stream->id);
#ifdef DUMP_STREAM
...
AddtoTimewaitList(mtcp_manager_t mtcp, tcp_stream *cur_stream, uint32_t cur_ts)
{
    ...
    if (cur_stream->on_timewait_list) {
        ...
        if (cur_stream->on_rto_idx >= 0) {
            TRACE_DEBUG("Stream %u: cannot be in both "
...
//	printf("%s-%d:cur_stream->sndvar->rto:%u,cur_ts:%u,cur_stream->sndvar->ts_rto:%u\n",__func__,__LINE__,cur_stream->sndvar->rto,cur_ts,cur_stream->sndvar->ts_rto);

        AddtoRTOList(mtcp, cur_stream);

    } else {
        /* all packets are acked */
        TRACE_DEBUG("All packets are acked. snd_una: %u, snd_nxt: %u\n", cur_stream->sndvar->snd_una, cur_stream->snd_nxt);
    }
}
/*----------------------------------------------------------------------------*/
int 
HandleRTO(mtcp_manager_t mtcp, uint32_t cur_ts, tcp_stream *cur_stream)
{
    ...
    TRACE_DEBUG("Stream %d Timeout! rto: %u (%ums), snd_una: %u, snd_nxt: %u\n", 
    ...
    if (cur_stream->sndvar->nrtx < TCP_MAX_RTX) {
        ...
        TRACE_DEBUG("Stream %d: Exceed MAX_RTX\n", cur_stream->id);
	...
	//printf("%s-%d: rto_prev:%u,cur_stream->sndvar->rto:%u,cur_stream->rcvvar->srtt:%u,cur_stream->rcvvar->rttvar:%u,backoff:%u\n",__func__,__LINE__,rto_prev,cur_stream->sndvar->rto,cur_stream->rcvvar->srtt,cur_stream->rcvvar->rttvar,backoff);
        if (cur_stream->sndvar->rto <= 0) {
            TRACE_DEBUG("Stream %d current rto: %u, prev: %u, state: %s\n", 
    ...
    TRACE_DEBUG("Stream %d Timeout. cwnd: %u, ssthresh: %u\n", 
    ...
    if (cur_stream->state == TCP_ST_SYN_SENT) {
        ...
        if (cur_stream->sndvar->nrtx > TCP_MAX_SYN_RETRY) {
            cur_stream->state = TCP_ST_CLOSED;
            cur_stream->close_reason = TCP_CONN_FAIL;
            TRACE_DEBUG("Stream %d: SYN retries exceed maximum retries.\n", 
                    cur_stream->id);
            if (cur_stream->socket) {
                RaiseErrorEvent(mtcp, cur_stream);
            } else {
        ...
        TRACE_DEBUG("Stream %d Retransmit SYN. snd_nxt: %u, snd_una: %u\n", 
                cur_stream->id, cur_stream->snd_nxt, cur_stream->sndvar->snd_una);

    } else if (cur_stream->state == TCP_ST_SYN_RCVD) {
        /* SYN/ACK lost */
        TRACE_DEBUG("Stream %d: Retransmit SYN/ACK. snd_nxt: %u, snd_una: %u\n", 
                cur_stream->id, cur_stream->snd_nxt, cur_stream->sndvar->snd_una);

    } else if (cur_stream->state == TCP_ST_ESTABLISHED) {
        /* Data lost */
        TRACE_DEBUG("Stream %d: Retransmit data. snd_nxt: %u, snd_una: %u\n", 
                cur_stream->id, cur_stream->snd_nxt, cur_stream->sndvar->snd_una);
        ...
        /* FIN/ACK lost */
        TRACE_DEBUG("Stream %d: Retransmit FIN/ACK. "
                "snd_nxt: %u, snd_una: %u\n", 
                cur_stream->id, cur_stream->snd_nxt, cur_stream->sndvar->snd_una);

    } else if (cur_stream->state == TCP_ST_FIN_WAIT_1) {
        /* FIN lost */
        TRACE_DEBUG("Stream %d: Retransmit FIN. snd_nxt: %u, snd_una: %u\n", 
                cur_stream->id, cur_stream->snd_nxt, cur_stream->sndvar->snd_una);
    } else if (cur_stream->state == TCP_ST_CLOSING) {
        TRACE_DEBUG("Stream %d: Retransmit ACK. snd_nxt: %u, snd_una: %u\n", 
                cur_stream->id, cur_stream->snd_nxt, cur_stream->sndvar->snd_una);
        //TRACE_DEBUG("Stream %d: Retransmitting at CLOSING\n", cur_stream->id);

    } else {
        TRACE_DEBUG("Stream %d: not implemented state! state: %s, rto: %u\n", 
                cur_stream->id, 
    ...
    if (cur_stream->have_reset &&
		...
		//printf("%s-%d:retransmission set cur_stream->snd_nxt:%u,cur_stream->sndvar->snd_una:%u,cur_stream->sndvar->tcp_data_ring.head:%u,cur_stream->sndvar->tcp_data_ring.una_head%u\n",__func__,__LINE__,cur_stream->snd_nxt,cur_stream->sndvar->snd_una,cur_stream->sndvar->tcp_data_ring.head,cur_stream->sndvar->tcp_data_ring.una_head);
    cur_stream->snd_nxt = cur_stream->sndvar->snd_una;
		cur_stream->sndvar->tcp_data_ring.head = cur_stream->sndvar->tcp_data_ring.una_head;
	...
	AddtoSendList(mtcp, cur_stream);
        ...
        if (cur_stream->sndvar->fss == 0) {
            TRACE_DEBUG("Stream %u: fss not set.\n", cur_stream->id);
        }
...
CheckRtmTimeout(mtcp_manager_t mtcp, uint32_t cur_ts, int thresh)
{
    ...
    while (1) {
        ...
        for (walk = TAILQ_FIRST(rto_list);
            ...
            TRACE_DEBUG("Inside rto list. cnt: %u, stream: %d\n", 
                    cnt, walk->id);
            ...
            if (walk->on_rto_idx >= 0) {
                ...
                mtcp->rto_list_cnt--;
                walk->on_rto_idx = -1;
		//printf("%s-%d: walk:%p,walk->state:%d\n",__func__,__LINE__,walk,walk->state);
		HandleRTO(mtcp, cur_ts, walk);
            } else {
                TRACE_DEBUG("Stream %d: not on rto list.\n", walk->id);
#ifdef DUMP_STREAM
    ...
    TRACE_DEBUG("Checking retransmission timeout. cnt: %d\n", cnt);
}
/*----------------------------------------------------------------------------*/
void 
CheckTimewaitExpire(mtcp_manager_t mtcp, uint32_t cur_ts, int thresh)
{
    ...
    for (walk = TAILQ_FIRST(&mtcp->timewait_list); 
        ...
        TRACE_DEBUG("Inside timewait list. cnt: %u, stream: %d\n", 
                cnt, walk->id);
        ...
        if (walk->on_timewait_list) {
            ...
            TRACE_DEBUG("Stream %d: not on timewait list.\n", walk->id);
#ifdef DUMP_STREAM
            DumpStream(mtcp, walk);
#endif
        }
    ...
    TRACE_DEBUG("Checking timewait timeout. cnt: %d\n", cnt);
}
/*----------------------------------------------------------------------------*/
void 

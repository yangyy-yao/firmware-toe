        ...
        uint8_t flags, uint8_t *tcpopt, uint16_t optlen)
{
    ...
    if (flags & TCP_FLAG_SYN) {
        ...
        TRACE_DEBUG("Local SACK permited.\n");
#endif /* TCP_OPT_SACK_ENABLED */
        ...
        uint32_t cur_ts, uint32_t echo_ts)
{
    ...
    if (payloadlen + optlen > TCP_DEFAULT_MSS) {
        TRACE_DEBUG("Payload size exceeds MSS.\n");
        assert(0);
        return ERROR;
    }
...
SendTCPPacket(struct mtcp_manager *mtcp, tcp_stream *cur_stream, 
        uint32_t cur_ts, uint8_t flags, uint8_t *payload, uint16_t payloadlen, int data_posit)
{
    struct tcphdr *tcph;
    uint16_t optlen;
    uint8_t wscale = 0;
    ...
    tcph = (struct tcphdr *)IPOutput(mtcp, cur_stream, 
            TCP_HEADER_LEN + optlen + payloadlen, payloadlen, data_posit);
    if (tcph == NULL) {
        TRACE_DEBUG("null tcph\n");
        return -2;
    }
    memset(tcph, 0, TCP_HEADER_LEN + optlen);
    ...
    if (flags & TCP_FLAG_SYN) {
        ...
        if (cur_stream->snd_nxt != cur_stream->sndvar->iss) {
            TRACE_DEBUG("Stream %d: weird SYN sequence. "
                    "snd_nxt: %u, iss: %u\n", cur_stream->id, 
                    cur_stream->snd_nxt, cur_stream->sndvar->iss);
        }

        TRACE_DEBUG("Stream %d: Sending SYN. seq: %u, ack: %u\n", cur_stream->id, cur_stream->snd_nxt, cur_stream->rcv_nxt);
    }
    if (flags & TCP_FLAG_RST) {
        TRACE_DEBUG("Stream %d: Sending RST.\n", cur_stream->id);
        tcph->rst = TRUE;
    }
    if (flags & TCP_FLAG_PSH)
        tcph->psh = TRUE;
    ...
    if (flags & TCP_FLAG_WACK) {
        ...
        TRACE_DEBUG("%u Sending ACK to get new window advertisement. "
        ...
        if (cur_stream->sndvar->fss == 0) {
            TRACE_DEBUG("Stream %u: not fss set. closed: %u\n", 
                    cur_stream->id, cur_stream->closed);
        }
        tcph->seq = htonl(cur_stream->sndvar->fss);
        cur_stream->sndvar->is_fin_sent = TRUE;
        TRACE_DEBUG("Stream %d: Sending FIN. seq: %u, ack_seq: %u\n", 
                cur_stream->id, cur_stream->snd_nxt, cur_stream->rcv_nxt);
    } else {
        TRACE_DEBUG("Stream %d: seq: %u\n", cur_stream->id, cur_stream->snd_nxt);
        tcph->seq = htonl(cur_stream->snd_nxt);
    }

    if (flags & TCP_FLAG_ACK) {
        printf("Stream %d: Sending ACK.  seq=%u, %u  ack=%u, %u\n", cur_stream->id, htonl(cur_stream->snd_nxt),cur_stream->snd_nxt, htonl(cur_stream->rcv_nxt),cur_stream->rcv_nxt);
        tcph->ack = TRUE;
    ...
    //printf("%s-%d: cur_stream->rcvvar->rcv_wnd:%u,wscale:%u\n",__func__,__LINE__,cur_stream->rcvvar->rcv_wnd,wscale);
...
#endif
   //printf("%s-%d: cur_stream->snd_nxt:%u, payloadlen:%d\n",__func__,__LINE__,cur_stream->snd_nxt,payloadlen); 
...
FlushTCPSendingBuffer(mtcp_manager_t mtcp, tcp_stream *cur_stream, uint32_t cur_ts)
{
 ...
 out:
    ...
    if (!sndvar->sndbuf) {
        TRACE_DEBUG("Stream %d: No send buffer available.\n", cur_stream->id);
        assert(0);
        return 0;
    }
    ...
    while (1) {
        ...
        //printf("%s-%d  len=%u,sndvar->sndbuf->len:%u,seq:%u,sndvar->sndbuf->head_seq:%u\n",__func__,__LINE__, len,sndvar->sndbuf->len,seq,sndvar->sndbuf->head_seq);
#ifdef ENABLE_CCP
        ...
        /* sanity check */
        if (TCP_SEQ_LT(seq, sndvar->sndbuf->head_seq)) {
            TRACE_DEBUG("Stream %d: Invalid sequence to send. "
                        "state: %s, seq: %u, head_seq: %u.\n",
                        cur_stream->id, TCPStateToString(cur_stream),
                        seq, sndvar->sndbuf->head_seq);
            assert(0);
        ...
        if (TCP_SEQ_LT(seq, sndvar->snd_una)) {
            printf("Stream %d: Invalid sequence to send. "
        ...
        remaining_window = MIN(sndvar->cwnd, sndvar->peer_wnd)
                           - (seq - sndvar->snd_una);
        //printf("%s-%d  remaining_window=%d,sndvar->cwnd:%u, sndvar->peer_wnd:%u,seq:%u,sndvar->snd_una:%u\n",__func__,__LINE__, remaining_window,sndvar->cwnd,sndvar->peer_wnd,seq,sndvar->snd_una);
        ...
        //printf("%s-%d: !!! cur_stream->sndvar->tcp_data_ring.una_head=%u,head:%u  tail=%u\n",__func__, __LINE__, cur_stream->sndvar->tcp_data_ring.una_head,cur_stream->sndvar->tcp_data_ring.head, cur_stream->sndvar->tcp_data_ring.tail);

	if (cur_stream->sndvar->tcp_data_ring.head == cur_stream->sndvar->tcp_data_ring.tail) {
		break;
	}
	
	data_posit = cur_stream->sndvar->tcp_data_ring.head;
	pkt_len = cur_stream->sndvar->tcp_data_ring.mbuf_data_len[data_posit];
	m = cur_stream->sndvar->tcp_data_ring.m[data_posit];
	if (!m) {
		
		printf("%s-%d:m is null\n",__func__,__LINE__);
		break;
	}

	printf("%s-%d:data_posit:%d,pkt_len:%u,len:%u\n",__func__,__LINE__,data_posit,pkt_len,len);
	if (pkt_len > len) {
		//printf("%s-%d:pkt_len:%u,len:%u\n",__func__,__LINE__,pkt_len,len);
		break;
	}
...
#endif
	//printf("%s-%d: send pkt,pkt_len:%d\n",__func__,__LINE__,pkt_len);
	if ((sndlen = SendTCPPacket(mtcp, cur_stream, cur_ts,
                        TCP_FLAG_ACK, data, pkt_len, data_posit)) < 0) {
            /* there is no available tx buf */
            packets = -3;
            goto out;
        }
	cur_stream->sndvar->tcp_data_ring.head = (cur_stream->sndvar->tcp_data_ring.head + 1) % TCP_SEND_DATA_BUFFER_MAX_NUM;
	cur_stream->sndvar->tcp_data_ring.free_num ++;
	if (cur_stream->sndvar->tcp_data_ring.free_num == TCP_SEND_DATA_BUFFER_MAX_NUM
			&& cur_stream->send_fin_masg.is_fin) {
		SendTCPPacket(mtcp, cur_stream, cur_ts, cur_stream->send_fin_masg.tcp_flag, NULL, 0, 0);
		cur_stream->connect_state = cur_stream->send_fin_masg.connect_state;
	}
#ifdef ENABLE_CCP
        if (sndvar->missing_seq) {
 ...
 out:
    //SBUF_UNLOCK(&sndvar->write_lock);   
    //printf("%s-%d  return packets=%d\n",__func__,__LINE__, packets);
    return packets; 
#endif
}
/*----------------------------------------------------------------------------*/
...
SendControlPacket(mtcp_manager_t mtcp, tcp_stream *cur_stream, uint32_t cur_ts)
{
	...
	if (cur_stream->state == TCP_ST_SYN_SENT) {
		/* Send SYN here */
		ret = SendTCPPacket(mtcp, cur_stream, cur_ts, TCP_FLAG_SYN, NULL, 0, 0);

	} else if (cur_stream->state == TCP_ST_SYN_RCVD) {
		/* Send SYN/ACK here */
		cur_stream->snd_nxt = sndvar->iss;
		ret = SendTCPPacket(mtcp, cur_stream, cur_ts, 
				TCP_FLAG_SYN | TCP_FLAG_ACK, NULL, 0, 0);

	} else if (cur_stream->state == TCP_ST_ESTABLISHED) {
		/* Send ACK here */
		ret = SendTCPPacket(mtcp, cur_stream, cur_ts, TCP_FLAG_ACK, NULL, 0, 0);
		...
		/* Send RST here */
		TRACE_DEBUG("Stream %d: Try sending RST (TCP_ST_CLOSED)\n", 
				cur_stream->id);
		/* first flush the data and ack */
		if (sndvar->on_send_list || sndvar->on_ack_list) {
			ret = -1;
		} else {
			ret = SendTCPPacket(mtcp, cur_stream, cur_ts, TCP_FLAG_RST, NULL, 0, 0);
			if (ret >= 0) {
		...
		struct mtcp_sender *sender, uint32_t cur_ts, int thresh)
{
	...
	while (cur_stream) {
		...
		TRACE_DEBUG("Inside control loop. cnt: %u, stream: %d\n", 
				cnt, cur_stream->id);
		...
		if (cur_stream->sndvar->on_control_list) {
			...
			TRACE_DEBUG("Stream %d: not on control list.\n", cur_stream->id);
		}
        ...
        struct mtcp_sender *sender, uint32_t cur_ts, int thresh)
{
    ...
    while (cur_stream) {
        ...
        TRACE_DEBUG("Inside send loop. cnt: %u, stream: %d\n", cnt, cur_stream->id);
        ...
        if (cur_stream->sndvar->on_send_list) {
            ...
            if (cur_stream->state == TCP_ST_ESTABLISHED) {
                if (cur_stream->sndvar->on_control_list) {
                    ...
                    //TRACE_DEBUG("Stream %u: delay sending data.\n", cur_stream->id);
                    ret = -1;
                } else {
                ...
                TRACE_DEBUG("Stream %d: on_send_list at state %s\n", 
                        cur_stream->id, TCPStateToString(cur_stream));
#if DUMP_STREAM
            ...
            TRACE_DEBUG("Stream %d: not on send list.\n", cur_stream->id);
#ifdef DUMP_STREAM
            DumpStream(mtcp, cur_stream);
#endif
        }
        ...
        struct mtcp_sender *sender, uint32_t cur_ts, int thresh)
{
    ...
    while (cur_stream) {
        ...
        TRACE_DEBUG("Inside ack loop. cnt: %u\n", cnt);
        ...
        if (cur_stream->sndvar->on_ack_list) {
            ...
            TRACE_DEBUG("Stream %d: not on ack list.\n", cur_stream->id);
            TAILQ_REMOVE(&sender->ack_list, cur_stream, sndvar->ack_link);
            sender->ack_list_cnt--;
#ifdef DUMP_STREAM
...
GetSender(mtcp_manager_t mtcp, tcp_stream *cur_stream)
{
    ...
    if (eidx < 0 || eidx >= CONFIG.eths_num) {
        TRACE_DEBUG("(NEVER HAPPEN) Failed to find appropriate sender.\n");
        return NULL;
    }
...
AddtoControlList(mtcp_manager_t mtcp, tcp_stream *cur_stream, uint32_t cur_ts)
{
    ...
    if (ret < 0) {
        ...
        if (!cur_stream->sndvar->on_control_list) {
            ...
            sender->control_list_cnt++;
            //TRACE_DEBUG("Stream %u: added to control list (cnt: %d)\n", 
...
AddtoSendList(mtcp_manager_t mtcp, tcp_stream *cur_stream)
{
    ...
    if(!cur_stream->sndvar->sndbuf) {
        TRACE_DEBUG("[%d] Stream %d: No send buffer available.\n", mtcp->ctx->cpu, cur_stream->id);
        assert(0);
        return;
    }

    //printf("%s-%d: cur_stream->sndvar->on_send_list:%u,sender->send_list_cnt:%d\n",__func__,__LINE__,cur_stream->sndvar->on_send_list,sender->send_list_cnt);
    if (!cur_stream->sndvar->on_send_list) {
...
{
    //struct mtcp_sender *sender = GetSender(mtcp, cur_stream);
    struct mtcp_sender *sender = mtcp->g_sender;
    assert(sender != NULL);
    //printf("%s-%d:cur_stream->sndvar->on_ack_list:%d \n",__func__,__LINE__,cur_stream->sndvar->on_ack_list);
...
RemoveFromControlList(mtcp_manager_t mtcp, tcp_stream *cur_stream)
{
    ...
    if (cur_stream->sndvar->on_control_list) {
        ...
        TAILQ_REMOVE(&sender->control_list, cur_stream, sndvar->control_link);
        sender->control_list_cnt--;
        //TRACE_DEBUG("Stream %u: Removed from control list (cnt: %d)\n", 
        //      cur_stream->id, sender->control_list_cnt);
    }
}
/*----------------------------------------------------------------------------*/
        ...
        tcp_stream *cur_stream, uint32_t cur_ts, uint8_t opt)
{
    if (!(cur_stream->state == TCP_ST_ESTABLISHED || 
            ...
            cur_stream->state == TCP_ST_FIN_WAIT_1 || 
            cur_stream->state == TCP_ST_FIN_WAIT_2)) {
        TRACE_DEBUG("Stream %u: Enqueueing ack at state %s\n", cur_stream->id, TCPStateToString(cur_stream));
    }
...
DumpControlList(mtcp_manager_t mtcp, struct mtcp_sender *sender)
{
    ...
    TRACE_DEBUG("Dumping control list (count: %d):\n", sender->control_list_cnt);
    TAILQ_FOREACH(stream, &sender->control_list, sndvar->control_link) {
        TRACE_DEBUG("Stream id: %u in control list\n", stream->id);
    }
}

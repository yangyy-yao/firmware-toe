...
struct fragment_ctx
{
    ...
    uint32_t is_calloc : 1;
		...
		struct rte_mbuf *tail_mbuf;
    struct fragment_ctx *next;
};
/*----------------------------------------------------------------------------*/
/*
#define TCP_RECV_MBUF_RING_MAX_NUM 32767
struct tcp_recv_mbuf {
		struct rte_mbuf *m_data[TCP_RECV_MBUF_RING_MAX_NUM];
		int head;
		int prev_head;
		int tail;
		int size;
...
struct tcp_ring_buffer
{
   ...
   // struct tcp_recv_mbuf data_buf;
    ...
    uint32_t head_seq;  //recv buffer中，还未给用户的起始数据序号
    uint32_t init_seq;
...
struct tcp_ring_buffer* RBInit_no_copy(mtcp_manager_t mtcp, uint32_t init_seq);

...
static void toe_rx_databuf_dma_process(struct toe_sync_dma_job *sjob, struct toe_engine *toe_eg)
{
	...
	stream->sndvar->tcp_data_ring.free_num -= sjob->meb_num;
...
//	uint16_t site;
	...
	for (j = 0; j < num; j++) {
		...
		printf("%s-%d:data_msg->data_len:%d,data_msg->identification.card_stream_addr:0x%llx \n",__func__,__LINE__,data_msg->data_len,data_msg->identification.card_stream_addr);
...
#else
		printf("%s-%d: m_num:%d, stream->sndvar->tcp_data_ring.free_num:%d\n",__func__,__LINE__,m_num,stream->sndvar->tcp_data_ring.free_num);

		m_final_num = m_num;
		if (stream->sndvar->tcp_data_ring.free_num < m_final_num) {
			RTE_LOG(ERR, PMD, "%s-%d: tcp data ring is full! stream->sndvar->tcp_data_ring.free_num:%d,m_final_num:%d\n", __func__, __LINE__,stream->sndvar->tcp_data_ring.free_num,m_final_num);
		...
		for (i = 0; i < m_num; i++) {
...
#endif
			printf("%s-%d:sjob[i]:%p,head_mbuf:%p,head_mbuf->nb_seg:%d,pre_mbuf:%p,idx:%d\n", __func__,__LINE__,sjob[i],head_mbuf, head_mbuf->nb_segs,pre_mbuf,idx);
			sjob[i]->meb_num = 0;
			sjob[i]->cb = NULL;
			sjob[i]->qid = idx;
			...
			printf("%s-%d:data_msg->data_len:%d,mbufs[i]->buf_len:%d,job_len:%d,en_len:%d\n", __func__,__LINE__,data_msg->data_len, mbufs[i]->buf_len, job_len,en_len);
			sjob[i]->job->cnxt = (uint64_t)sjob[i];
			sjob[i]->job->src = src_addr;
			sjob[i]->job->dest = dst_addr;
			sjob[i]->job->len = job_len;
...
static void toe_tx_data_reply_dma_process(struct toe_sync_dma_job *sjob, struct toe_engine *toe_eg)
{
	...
	printf("%s-%d:sjob->qid:%d,sjob->meb_num:%d, data_cq->head:%d,  data_cq->tail:%d,data_cq->cq_local->rq_head:%d,loop_count:%llu\n",__func__,__LINE__,sjob->qid,sjob->meb_num, data_cq->head, data_cq->tail, data_cq->cq_local->rq_head,loop_count);
...
static void toe_tx_databuf_dma_process(struct toe_sync_dma_job *sjob, struct toe_engine *toe_eg)
{
	...
	printf("%s-%d: stream->rcvvar->rcvbuf->data_buf.head:%d\n",__func__, __LINE__,stream->rcvvar->rcvbuf->data_buf.head);
	stream->ref_count --;
...
int toe_rx_data_dma_enqueue(struct toe_engine *toe_eg, int idx)
{
	...
	printf("%s-%d:data_rq->rbc->doorbell:%d,data_rq->tai:%d,data_rq->pre_head:%d\n",__func__,__LINE__,data_rq->rbc->doorbell,data_rq->tail,data_rq->pre_head);

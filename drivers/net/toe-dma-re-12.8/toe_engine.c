...
uint8_t *toe_get_wptr_datapkt(struct mtcp_thread_context *ctx, uint16_t len, void *st, int data_posit)
{
	...
	if (!diff_len)
		...
		printf("%s-%d:data len err!! len:%u	mbuf->data_len:%u,stream->sndvar->tcp_data_ring.mbuf_data_len[data_posit]:%u\n",__func__,__LINE__,len, mbuf->data_len, stream->sndvar->tcp_data_ring.mbuf_data_len[data_posit]);
		return NULL;
	}

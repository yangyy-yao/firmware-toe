...
int toe_rx_databuf_dma_enqueue(struct toe_engine *toe_eg, int idx)
{
  ...
  printf("%s-%d: data_rq->real_tail:%d, data_rq->head:%d,  Doorbell:%u\n", __func__, __LINE__, data_rq->real_tail,data_rq->head, data_rq->rbc->doorbell);	
...
static void toe_dma_hwq_init(int pf, int vf)
{
	...
	lcore_id = strtol(core_id, NULL, 10);
	if (lcore_id >= RTE_MAX_LCORE) {
		RTE_LOG(ERR, PMD, TOE_CTRL_PROCESS ":%s,%d value is err\n",core_id, lcore_id);
		return -1;
	}*/
	RTE_LCORE_FOREACH(lcore) {
	//for (lcore = 0; lcore < RTE_MAX_LCORE; lcore++) {
	//	if (lcore !=lcore_id && rte_lcore_is_enabled(lcore) == 0)
	//		continue;
		hwq = &tdma_hwq[lcore];
		hwq->lcore_id = lcore;
		hwq->id = tqdma_dev_id;
		...
		assert(hwq->vq >= 0);
		printf("%s-%d: hwq->vq:%d,lcore:%d\n",__func__,__LINE__,hwq->vq,lcore);
		//for (i = 0; i < pf_num; i++) {
			//pf = pfs[i];

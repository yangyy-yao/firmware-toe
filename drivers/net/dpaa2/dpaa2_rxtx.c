...
dpaa2_dev_rx_parse_new(struct rte_mbuf *m, const struct qbman_fd *fd)
{
	...
	if ((m->packet_type && RTE_PTYPE_L4_UDP) || (m->packet_type && RTE_PTYPE_L4_TCP))
...
dpaa2_dev_prefetch_rx(void *queue, struct rte_mbuf **bufs, uint16_t nb_pkts)
{
	...
	while (!qbman_check_command_complete(dq_storage))
		;
	rte_atomic64_cmpset(
		(uint64_t *)get_swp_active_dqs_addr(q_storage->active_dpio_id),
		(uint64_t)dq_storage, (uint64_t)NULL);

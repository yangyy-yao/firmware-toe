	...
	struct proto_discrimination proto, int group)
{
	...
	key_iova = flow->qos_key_addr + offset;
	mask_iova = flow->qos_mask_addr + offset;
	if (proto.type == RTE_FLOW_ITEM_TYPE_ETH) {
	...
	key_iova = flow->fs_key_addr + offset;
	mask_iova = flow->fs_mask_addr + offset;
...
dpaa2_flow_rule_data_set(
	struct dpaa2_key_extract *key_extract,
	struct dpni_rule_cfg *rule __rte_unused,
	enum net_prot prot, uint32_t field,
	const void *key, const void *mask, int size,
	struct rte_flow *flow, int dist_type)
{
...
_dpaa2_flow_rule_move_ipaddr_tail(
	struct dpaa2_key_extract *key_extract,
	struct dpni_rule_cfg *rule __rte_unused, int src_offset,
	...
	struct rte_flow *flow, int dist_type)
{
	...
	if (dist_type & DPAA2_QOS_TABLE_RECONFIGURE) {
		key_src = flow->qos_key_addr + src_offset;
		mask_src = flow->qos_mask_addr + src_offset;
		key_dst = flow->qos_key_addr + dst_offset;
		mask_dst = flow->qos_mask_addr + dst_offset;
	}

	if (dist_type & DPAA2_FS_TABLE_RECONFIGURE) {
		key_src = flow->fs_key_addr + src_offset;
		mask_src = flow->fs_mask_addr + src_offset;
		key_dst = flow->fs_key_addr + dst_offset;
		mask_dst = flow->fs_mask_addr + dst_offset;
	}
	...
	int fs_group)
{
	...
	if (flow->ipaddr_rule.qos_ipsrc_offset >= 0) {
		ret = _dpaa2_flow_rule_move_ipaddr_tail(
				...
				&flow->qos_rule,
				flow->ipaddr_rule.qos_ipsrc_offset,
				NH_FLD_IP_SRC, prot == NET_PROT_IPV4, flow, DPAA2_QOS_TABLE_RECONFIGURE);
		if (ret) {
	...
	if (flow->ipaddr_rule.qos_ipdst_offset >= 0) {
		ret = _dpaa2_flow_rule_move_ipaddr_tail(
				...
				&flow->qos_rule,
				flow->ipaddr_rule.qos_ipdst_offset,
				NH_FLD_IP_DST, prot == NET_PROT_IPV4, flow, DPAA2_QOS_TABLE_RECONFIGURE);
		if (ret) {
	...
	if (flow->ipaddr_rule.fs_ipsrc_offset >= 0) {
		ret = _dpaa2_flow_rule_move_ipaddr_tail(
				...
				&flow->fs_rule,
				flow->ipaddr_rule.fs_ipsrc_offset,
				NH_FLD_IP_SRC, prot == NET_PROT_IPV4, flow, DPAA2_FS_TABLE_RECONFIGURE);
		if (ret) {
	...
	if (flow->ipaddr_rule.fs_ipdst_offset >= 0) {
		ret = _dpaa2_flow_rule_move_ipaddr_tail(
				...
				&flow->fs_rule,
				flow->ipaddr_rule.fs_ipdst_offset,
				NH_FLD_IP_DST, prot == NET_PROT_IPV4, flow, DPAA2_FS_TABLE_RECONFIGURE);
		if (ret) {
			 ...
			 int *device_configured)
{
	...
	if (memcmp((const char *)&mask->src, zero_cmp, RTE_ETHER_ADDR_LEN)) {
		...
		ret = dpaa2_flow_rule_data_set(
				...
				&spec->src.addr_bytes,
				&mask->src.addr_bytes,
				sizeof(struct rte_ether_addr), flow, DPAA2_QOS_TABLE_RECONFIGURE);
		if (ret) {
		...
		ret = dpaa2_flow_rule_data_set(
				...
				&spec->src.addr_bytes,
				&mask->src.addr_bytes,
				sizeof(struct rte_ether_addr), flow, DPAA2_FS_TABLE_RECONFIGURE);
		if (ret) {
	...
	if (memcmp((const char *)&mask->type, zero_cmp, sizeof(rte_be16_t))) {
		...
		ret = dpaa2_flow_rule_data_set(
				...
				&spec->type,
				&mask->type,
				sizeof(rte_be16_t), flow, DPAA2_QOS_TABLE_RECONFIGURE);
		if (ret) {
		...
		ret = dpaa2_flow_rule_data_set(
				...
				&spec->type,
				&mask->type,
				sizeof(rte_be16_t), flow, DPAA2_FS_TABLE_RECONFIGURE);
		if (ret) {
	...
	int *device_configured)
{
	...
	if ((mask_ipv4 && mask_ipv4->hdr.src_addr) ||
		...
		ret = dpaa2_flow_rule_data_set(
				...
				&flow->qos_rule,
				prot, NH_FLD_IP_SRC,
				key,	mask, size, flow, DPAA2_QOS_TABLE_RECONFIGURE);
		if (ret) {
		...
		ret = dpaa2_flow_rule_data_set(
				...
				&flow->fs_rule,
				prot, NH_FLD_IP_SRC,
				key,	mask, size, flow, DPAA2_FS_TABLE_RECONFIGURE);
		if (ret) {
	...
	if ((mask_ipv4 && mask_ipv4->hdr.next_proto_id) ||
		...
		ret = dpaa2_flow_rule_data_set(
				...
				NET_PROT_IP,
				NH_FLD_IP_PROTO,
				key,	mask, NH_FLD_IP_PROTO_SIZE, flow, DPAA2_QOS_TABLE_RECONFIGURE);
		if (ret) {
		...
		ret = dpaa2_flow_rule_data_set(
				...
				NET_PROT_IP,
				NH_FLD_IP_PROTO,
				key,	mask, NH_FLD_IP_PROTO_SIZE, flow, DPAA2_FS_TABLE_RECONFIGURE);
		if (ret) {
			  ...
			  int *device_configured)
{
	...
	if (mask->hdr.icmp_type) {
		...
		ret = dpaa2_flow_rule_data_set(
				...
				&spec->hdr.icmp_type,
				&mask->hdr.icmp_type,
				NH_FLD_ICMP_TYPE_SIZE, flow, DPAA2_QOS_TABLE_RECONFIGURE);
		if (ret) {
		...
		ret = dpaa2_flow_rule_data_set(
				...
				&spec->hdr.icmp_type,
				&mask->hdr.icmp_type,
				NH_FLD_ICMP_TYPE_SIZE, flow, DPAA2_FS_TABLE_RECONFIGURE);
		if (ret) {
	...
	if (mask->hdr.icmp_code) {
		...
		ret = dpaa2_flow_rule_data_set(
				...
				&spec->hdr.icmp_code,
				&mask->hdr.icmp_code,
				NH_FLD_ICMP_CODE_SIZE, flow, DPAA2_QOS_TABLE_RECONFIGURE);
		if (ret) {
		...
		ret = dpaa2_flow_rule_data_set(
				...
				&spec->hdr.icmp_code,
				&mask->hdr.icmp_code,
				NH_FLD_ICMP_CODE_SIZE, flow, DPAA2_FS_TABLE_RECONFIGURE);
		if (ret) {
			  ...
			  int *device_configured)
{
	...
	if (mask->hdr.src_port) {
		...
		ret = dpaa2_flow_rule_data_set(&priv->extract.qos_key_extract,
				...
				&spec->hdr.src_port,
				&mask->hdr.src_port,
				NH_FLD_UDP_PORT_SIZE, flow, DPAA2_QOS_TABLE_RECONFIGURE);
		if (ret) {
			DPAA2_PMD_ERR(
		...
		ret = dpaa2_flow_rule_data_set(
				...
				&spec->hdr.src_port,
				&mask->hdr.src_port,
				NH_FLD_UDP_PORT_SIZE, flow, DPAA2_FS_TABLE_RECONFIGURE);
		if (ret) {
			DPAA2_PMD_ERR(
			 ...
			 int *device_configured)
{
	...
	if (mask->hdr.src_port) {
		...
		ret = dpaa2_flow_rule_data_set(
				...
				&spec->hdr.src_port,
				&mask->hdr.src_port,
				NH_FLD_TCP_PORT_SIZE, flow, DPAA2_QOS_TABLE_RECONFIGURE);
		if (ret) {
			DPAA2_PMD_ERR(
		...
		ret = dpaa2_flow_rule_data_set(
				...
				&spec->hdr.src_port,
				&mask->hdr.src_port,
				NH_FLD_TCP_PORT_SIZE, flow, DPAA2_FS_TABLE_RECONFIGURE);
		if (ret) {
			DPAA2_PMD_ERR(
			  ...
			  int *device_configured)
{
	...
	if (mask->hdr.src_port) {
		...
		ret = dpaa2_flow_rule_data_set(
				...
				&spec->hdr.src_port,
				&mask->hdr.src_port,
				NH_FLD_SCTP_PORT_SIZE, flow, DPAA2_QOS_TABLE_RECONFIGURE);
		if (ret) {
			DPAA2_PMD_ERR(
		...
		ret = dpaa2_flow_rule_data_set(
				...
				&spec->hdr.src_port,
				&mask->hdr.src_port,
				NH_FLD_SCTP_PORT_SIZE, flow, DPAA2_FS_TABLE_RECONFIGURE);
		if (ret) {
			DPAA2_PMD_ERR(
	...
	struct dpaa2_dev_priv *priv, uint8_t tc_id)
{
	...
	while (curr) {
		...
		if (curr->ipaddr_rule.qos_ipsrc_offset >= 0) {
			...
			memcpy(ipsrc_key,
				(char *)curr->qos_key_addr +
				curr->ipaddr_rule.qos_ipsrc_offset,
				size);
			memset((char *)curr->qos_key_addr +
				curr->ipaddr_rule.qos_ipsrc_offset,
				0, size);

			memcpy(ipsrc_mask,
				(char *)curr->qos_mask_addr +
				curr->ipaddr_rule.qos_ipsrc_offset,
				size);
			memset((char *)curr->qos_mask_addr +
				curr->ipaddr_rule.qos_ipsrc_offset,
				0, size);
		...
		if (curr->ipaddr_rule.qos_ipsrc_offset >= 0) {
			RTE_ASSERT((size == NH_FLD_IPV4_ADDR_SIZE) ||
				(size == NH_FLD_IPV6_ADDR_SIZE));
			memcpy((char *)curr->qos_key_addr +
				curr->ipaddr_rule.qos_ipsrc_offset,
				ipsrc_key,
				size);
			memcpy((char *)curr->qos_mask_addr +
				curr->ipaddr_rule.qos_ipsrc_offset,
				ipsrc_mask,
				size);
		}
		...
		if (curr->ipaddr_rule.fs_ipsrc_offset >= 0 &&
			...
			memcpy(ipsrc_key,
				(char *)curr->fs_key_addr +
				curr->ipaddr_rule.fs_ipsrc_offset,
				size);
			memset((char *)curr->fs_key_addr +
				curr->ipaddr_rule.fs_ipsrc_offset,
				0, size);

			memcpy(ipsrc_mask,
				(char *)curr->fs_mask_addr +
				curr->ipaddr_rule.fs_ipsrc_offset,
				size);
			memset((char *)curr->fs_mask_addr +
				curr->ipaddr_rule.fs_ipsrc_offset,
				0, size);
		...
		if (curr->ipaddr_rule.fs_ipsrc_offset >= 0) {
			memcpy((char *)curr->fs_key_addr +
				curr->ipaddr_rule.fs_ipsrc_offset,
				ipsrc_key,
				size);
			memcpy((char *)curr->fs_mask_addr +
				curr->ipaddr_rule.fs_ipsrc_offset,
				ipsrc_mask,
				size);
		}
	...
	while (!end_of_list) {
		...
		case RTE_FLOW_ACTION_TYPE_QUEUE:
			...
			if (is_keycfg_configured & DPAA2_FS_TABLE_RECONFIGURE) {
				...
				key_cfg_buf = (uint8_t *)(size_t)priv->extract.tc_extract_param[flow->tc_id];
				tc_cfg.key_cfg_iova = DPAA2_VADDR_TO_IOVA(key_cfg_buf);
			...
			if (is_keycfg_configured & DPAA2_QOS_TABLE_RECONFIGURE) {
				...
				key_cfg_buf = (uint8_t *)(size_t)priv->extract.qos_extract_param;
				qos_cfg.key_cfg_iova = DPAA2_VADDR_TO_IOVA(key_cfg_buf);
			...
			if (flow->tc_index >= priv->fs_entries) {
				DPAA2_PMD_ERR("FS table with %d entries full,flow->tc_index:%d",
					priv->fs_entries,flow->tc_index);
				return -1;
			}
		...
		case RTE_FLOW_ACTION_TYPE_RSS:
			...
			key_cfg_buf = (uint8_t *)(size_t)param;
			tc_cfg.key_cfg_iova = DPAA2_VADDR_TO_IOVA(key_cfg_buf);

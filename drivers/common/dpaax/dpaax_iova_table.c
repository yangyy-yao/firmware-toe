...
static int
dpaax_memevent_contig_walk_memsegs(const struct rte_memseg_list *msl __rte_unused,
...
static int
dpaax_memevent_walk_memsegs(const struct rte_memseg_list *msl,
	const struct rte_memseg *ms, __rte_unused void *arg )
{
	...
	DPAAX_DEBUG("Walking for %p (pa=%"PRIu64") and len %zu",
		    ms->addr, ms->phys_addr, ms->len);
	dpaax_iova_table_update(rte_mem_virt2iova(ms->addr), ms->addr, ms->len);
	return 0;
}
...
dpaax_handle_memevents(void)
{
	...
	if (rte_eal_iova_mode() == RTE_IOVA_VA)
		rte_memseg_contig_walk_thread_unsafe(dpaax_memevent_contig_walk_memsegs, NULL);
	else
		rte_memseg_walk_thread_unsafe(dpaax_memevent_walk_memsegs, NULL);

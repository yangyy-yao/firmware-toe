			...
			struct rte_qdma_rbp *rbp, int ser)
{
	...
	if (rbp->svfid != 0) {
		fd->simple_pci.svfa = 1;
		fd->simple_pci.svfid = rbp->svfid - 1;
	}
	fd->simple_pci.spfid = rbp->spfid;
	//fd->simple_pci.dvfid = rbp->dvfid;
	if (rbp->dvfid != 0) {
		fd->simple_pci.dvfa = 1;
		fd->simple_pci.dvfid = rbp->dvfid - 1;
	}
			...
			size_t len, uint32_t flags, uint32_t fmt)
{
	...
	if (rbp && rbp->enable) {
		...
		if (rbp->svfid != 0) {
			sdd->rbpcmd_simple.vfid = rbp->svfid - 1;
			sdd->rbpcmd_simple.vfa = 1;
		}
		...
		if (rbp->dvfid != 0) {
			sdd->rbpcmd_simple.vfid = rbp->dvfid - 1;
			sdd->rbpcmd_simple.vfa = 1;
		}
		...
		uint16_t nb_jobs)
{
	...
	uint32_t ctrl_field_F;
	uint32_t ctrl_field_N;
	struct qdma_sg_entry tmp;

	tmp.ctrl.f = 0;
	tmp.ctrl.sl = QDMA_SG_SL_LONG;
	tmp.ctrl.fmt = QDMA_SG_FMT_SDB;
	tmp.ctrl.offset = 0;
#ifdef RTE_LIBRTE_DPAA2_USE_PHYS_IOVA
	tmp.ctrl.bmt = QDMA_SG_BMT_ENABLE;
#else
	tmp.ctrl.bmt = QDMA_SG_BMT_DISABLE;
#endif
	tmp.ctrl.ivp = 0;
	tmp.ctrl.bpid = 0;
	ctrl_field_N = tmp.ctrl_fields;
	tmp.ctrl.f = QDMA_SG_F;
	ctrl_field_F = tmp.ctrl_fields;

	for (i = 0; i < nb_jobs; i++) {
		if (likely(i % 8 == 7)) {
			rte_prefetch0(&jobs[i + 1]);
		}
		ppjob[i] = jobs[i];
		/* source SG */
		src_sge->addr_lo = (uint32_t)jobs[i]->src;
		src_sge->addr_hi = (jobs[i]->src >> 32) & 0x0001FFFF;

		src_sge->data_len.data_len_sl0 = jobs[i]->len;

		/* destination SG */
		dst_sge->addr_lo = (uint32_t)jobs[i]->dest;
		dst_sge->addr_hi = (jobs[i]->dest >> 32) & 0x0001FFFF;
		...
		if (i == (nb_jobs - 1)) {
			src_sge->ctrl_fields = ctrl_field_F;
			dst_sge->ctrl_fields = ctrl_field_F;
		} else {
			src_sge->ctrl_fields = ctrl_field_N;
			dst_sge->ctrl_fields = ctrl_field_N;
		}
		src_sge++;
		dst_sge++;
	}
		...
		uint16_t nb_jobs)
{
	...
	// ALL job must with same rbp
	if (job[0]->rbp)
		rbp = job[0]->rbp;

	/* Set the metadata */
	/* Save job context. */
	elem =(void *)((char *)elem + QDMA_FLE_SG_OFF);
	elem_iova += QDMA_FLE_SG_OFF;
	*((uint16_t *)((uint64_t)elem + QDMA_FLE_JOB_NB_OFFSET)) = nb_jobs;
	...
	if (likely(nb_jobs > 1)) {
		...
		len = qdma_populate_sg_entry(ppjob, job, src_sge, dst_sge, nb_jobs);
						...
						uint16_t *nb_jobs)
{
	...
	*nb_jobs = *((uint16_t *)((uint64_t)fle -
	...
	job[i - 1]->status = status;

	/* Free FLE to the pool */
	rte_mempool_put(qdma_vq->fle_pool,
				(void *)((uint64_t)fle - QDMA_FLE_FLE_OFFSET - QDMA_FLE_SG_OFF));
			  ...
			  rte_rawdev_obj_t queue_conf)
{
	...
	snprintf(pool_name, sizeof(pool_name),
		"fle_pool_%d_%d", rawdev->dev_id, i);

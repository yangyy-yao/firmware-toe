...
int ccp_init(struct ccp_datapath *dp) {
    ...
    datapath = (struct ccp_datapath*)rte_zmalloc(NULL, sizeof(struct ccp_datapath), RTE_CACHE_LINE_SIZE);
    if (!datapath) {
        return -1;
    }
    ...
    ccp_active_connections = (struct ccp_connection*)rte_zmalloc(NULL, MAX_NUM_CONNECTIONS * sizeof(struct ccp_connection), RTE_CACHE_LINE_SIZE);
    ...
    datapath_programs = (struct DatapathProgram*)rte_zmalloc(NULL, MAX_NUM_PROGRAMS * sizeof(struct DatapathProgram), RTE_CACHE_LINE_SIZE);
    if (!datapath_programs) {
        rte_free(datapath);
        rte_free(ccp_active_connections);
        return -1;
    }
...
struct ccp_connection *ccp_connection_start(void *impl, struct ccp_datapath_info *flow_info) {
    ...
    if (ok < 0) {
        TRACE_DEBUG("failed to send create message: %d\n", ok);
        return conn;
    }
...
inline void *ccp_get_global_impl(void) {
    return datapath->impl;
}

inline int ccp_set_global_impl(void *ptr) {
    datapath->impl = ptr;
    return 0;
}
...
int ccp_invoke(struct ccp_connection *conn) {
    ...
    if (!(state->sent_create)) {
        ...
        // index of pointer back to this sock for IPC callback
        TRACE_DEBUG("%s retx create message\n", __FUNCTION__);
        ok = send_conn_create(datapath, conn);
        if (ok < 0) {
            TRACE_DEBUG("failed to retx create message: %d\n", ok);
        } else {
            ccp_conn_create_success(state);
        }
    ...
    for (i = 0; i < MAX_CONTROL_REG; i++) {
        if (state->pending_update.control_is_pending[i]) {
            TRACE_DEBUG("[sid=%d] Applying staged field update: control reg %u (%d->%d) \n", conn->index, i,
    ...
    if (state->pending_update.impl_is_pending[CWND_REG]) {
        TRACE_DEBUG("[sid=%d] Applying staged field update: cwnd reg <- %llu\n", conn->index, state->pending_update.impl_registers[CWND_REG]);
    ...
    if (state->pending_update.impl_is_pending[RATE_REG]) {
        TRACE_DEBUG("[sid=%d] Applying staged field update: rate reg <- %llu\n", conn->index, state->pending_update.impl_registers[RATE_REG]);
...
struct ccp_connection *ccp_connection_lookup(u16 sid) {
    ...
    if (sid == 0 || sid > MAX_NUM_CONNECTIONS) {
        TRACE_DEBUG("index out of bounds: %d", sid);
        return NULL;
    }

    conn = &ccp_active_connections[sid-1];
    if (conn->index != sid) {
        TRACE_DEBUG("index mismatch: sid %d, index %d", sid, conn->index);
        return NULL;
    }
...
void ccp_connection_free(u16 sid) {
    ...
    if (ok < 0) {
        TRACE_DEBUG("error sending close message: %d", ok);
    }
...
struct DatapathProgram* datapath_program_lookup(u16 pid) {
    ...
    // bounds check
    if (pid == 0) {
        TRACE_DEBUG("no datapath program set\n");
        return NULL;
    } else if (pid > MAX_NUM_PROGRAMS) {
        TRACE_DEBUG("program index out of bounds: %d\n", pid);
        return NULL;
    }

    prog = &datapath_programs[pid-1];
    if (prog->index != pid) {
        TRACE_DEBUG("index mismatch: pid %d, index %d", pid, prog->index);
        return NULL;
    }
...
int datapath_program_install(struct InstallExpressionMsgHdr* install_expr_msg, char* buf) {
    ...
    TRACE_DEBUG("Trying to install new program with (uid=%d) with %d expressions and %d instructions\n", program->program_uid, program->num_expressions, program->num_instructions);
    ...
    for (i=0; i < (int)(program->num_instructions); i++) {
        ...
        if (ok < 0) {
            TRACE_DEBUG("Could not read instruction # %d: %d in program with uid %u\n", i, ok, program->program_uid);
            return ok;
        }
        msg_ptr += sizeof(struct InstructionMsg);
    }

    TRACE_DEBUG("installed new program (uid=%d) with %d expressions and %d instructions\n", program->program_uid, program->num_expressions, program->num_instructions);
...
void datapath_program_free(u16 pid) {
    ...
    if (pid == 0 || pid > MAX_NUM_PROGRAMS) {
        TRACE_DEBUG("index out of bounds: %d", pid);
        return;
    }

    program = &datapath_programs[pid-1];
    if (program->index != pid) {
        TRACE_DEBUG("index mismatch: pid %d, index %d", pid, program->index);
        return;
    }
...
int ccp_read_msg(
    ...
    if (ok < 0) {
        TRACE_DEBUG("read header failed: %d", ok);
        return -1;
    }

    if (bufsize < 0) {
        TRACE_DEBUG("negative bufsize: %d", bufsize);
        return -2;
    }
    if (hdr.Len > ((u32) bufsize)) {
        TRACE_DEBUG("message size wrong: %u > %d\n", hdr.Len, bufsize);
        return -3;
    }

    if (hdr.Len > BIGGEST_MSG_SIZE) {
        TRACE_DEBUG("message too long: %u > %d\n", hdr.Len, BIGGEST_MSG_SIZE);
        return -4;
    }
    msg_ptr = buf + ok;
    ...
    if (hdr.Type == INSTALL_EXPR) {
        ...
        if ( ok < 0 ) {
            TRACE_DEBUG("could not install datapath program: %d\n", ok);
            return -6;
        }
    ...
    state = get_ccp_priv_state(conn);
    if (conn == NULL) {
        TRACE_DEBUG("unknown connection: %u\n", hdr.SocketId);
        return -7;
    }

    if (hdr.Type == UPDATE_FIELDS) {
        TRACE_DEBUG("[sid=%d] Received update_fields message\n", conn->index);
        ok = check_update_fields_msg(&hdr, &num_updates, msg_ptr);
        msg_ptr += ok;
        if (ok < 0) {
            TRACE_DEBUG("Update fields message failed: %d\n", ok);
            return -8;
        }

        ok = stage_multiple_updates(&state->pending_update, num_updates, (struct UpdateField*) msg_ptr);
        if (ok < 0) {
            TRACE_DEBUG("Failed to stage updates: %d\n", ok);
            return -11;
        }

        TRACE_DEBUG("Staged %u updates\n", num_updates);
    } else if (hdr.Type == CHANGE_PROG) {
        TRACE_DEBUG("[sid=%d] Received change_prog message\n", conn->index);
        // check if the program is in the program_table
        ok = read_change_prog_msg(&hdr, &change_program, msg_ptr);
        if (ok < 0) {
            TRACE_DEBUG("Change program message deserialization failed: %d\n", ok);
            return -9;
        }
        msg_ptr += ok;
        ...
        if (msg_program_index < 0) {
            ...
            TRACE_DEBUG("Could not find datapath program with program uid: %u\n", msg_program_index);
            return -10;
        }
        ...
        TRACE_DEBUG("Staged switch to program %d\n", change_program.program_uid);
    }

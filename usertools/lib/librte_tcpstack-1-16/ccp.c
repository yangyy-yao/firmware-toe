...
#endif
#else
    TRACE_DEBUG("unable to set rate, both PACING and RATE_LIMIT are disabled."
...
_dp_send_msg(struct ccp_datapath *dp, struct ccp_connection *conn, char *msg, int msg_size)
{
    ...
    if (ret < 0) {
        TRACE_DEBUG("failed to send msg to ccp: %s\n", strerror(errno));
    }
    return ret;
}
/*----------------------------------------------------------------------------*/
...
setup_ccp_connection(mtcp_manager_t mtcp)
{
    ...
    if (ret < 0 && errno != EEXIST) {
        TRACE_DEBUG("Failed to create path for ccp unix socket (%d): %s\n",
                ret, strerror(errno));
    }
    ...
    if ((recv_sock = socket(AF_UNIX, SOCK_DGRAM, 0)) == -1) {
        TRACE_DEBUG("Failed to create unix recv socket for ccp comm\n");
        exit(EXIT_FAILURE);
    }
    ...
    if (bind(recv_sock, (struct sockaddr *)&local, path_len) == -1) {
        TRACE_DEBUG("(Cpu %d) failed to bind to unix://%s because %s\n",
    ...
    if (ccp_init(&dp) < 0) {
        TRACE_DEBUG("Failed to initialize ccp connection map\n");
        exit(EXIT_FAILURE);
    }
}
/*----------------------------------------------------------------------------*/
...
setup_ccp_send_socket(mtcp_manager_t mtcp)
{
    ...
    if ((send_sock = socket(AF_UNIX, SOCK_DGRAM, 0)) == -1) {
        TRACE_DEBUG("failed to create unix send socket for ccp comm\n");
        exit(EXIT_FAILURE);
    }
    ...
    if (connect(send_sock, (struct sockaddr *)&remote, path_len) == -1) {
        TRACE_DEBUG("failed to connect to unix://%s because %s\n", TO_CCP_PATH, strerror(errno));
        exit(EXIT_FAILURE);
    }
...
ccp_create(mtcp_manager_t mtcp, tcp_stream *stream)
{
    ...
    if (stream->ccp_conn == NULL) {
        TRACE_DEBUG("failed to initialize ccp_connection");
            } else {
        TRACE_DEBUG("ccp.create(%d)\n", stream->ccp_conn->index);
    }
}
         ...
         uint32_t ack, uint64_t bytes_delivered, uint64_t packets_delivered)
{
    ...
    */
    ...
    if (conn != NULL) {
        ...
        TRACE_DEBUG("ccp_connection not initialized\n");    
    }
}
...
ccp_record_event(mtcp_manager_t mtcp, tcp_stream *stream, uint8_t event_type, uint32_t val)
{
    ...
    switch(event_type) {
        ...
        break;
        case EVENT_ECN:
        TRACE_DEBUG("ecn is not currently supported!\n");
        break;
        default:
        TRACE_DEBUG("unknown record event type %d!\n", event_type);
        break;
    }
}
/*----------------------------------------------------------------------------*/

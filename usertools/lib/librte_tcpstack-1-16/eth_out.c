        ...
        int nif, unsigned char* dst_haddr, uint16_t iplen, tcp_stream *stream, uint16_t payloadlen, int data_posit)
{
     ...
     */
     /*
    if (nif < 0) {
        TRACE_DEBUG("No interface set!\n");
        return NULL;
    }

    eidx = CONFIG.nif_to_eidx[nif];
    if (eidx < 0) {
        TRACE_DEBUG("No interface selected!\n");
        return NULL;
    }
		*/
    if (!stream) {
		...
		if (payloadlen > 0)
    	buf = mtcp->iom->get_wptr_datapkt(mtcp->ctx, iplen + ETHERNET_HEADER_LEN, stream, data_posit);
		else {
			buf = mtcp->iom->get_wptr(mtcp->ctx, iplen + ETHERNET_HEADER_LEN);

		}
		
    if (!buf) {
        //TRACE_DEBUG("Failed to get available write buffer\n");
        return NULL;
    }
    memset(buf, 0, ETHERNET_HEADER_LEN + iplen - payloadlen);

#if 0
    TRACE_DEBUG("dst_hwaddr: %02X:%02X:%02X:%02X:%02X:%02X\n",
    ...
    for (i = 0; i < ETH_ALEN; i++) {
        ...
        //ethh->h_dest[i] = dst_haddr[i];
				...
				ethh->h_source[i] = stream->src_mac[i];
				ethh->h_dest[i] = stream->dst_mac[i];
    }

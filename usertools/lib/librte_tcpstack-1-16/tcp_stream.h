...
#endif
	struct tcp_send_data_ring tcp_data_ring;
	struct tcp_prepare_read_list pre_read_list;
...
typedef struct tcp_stream
{
    ...
    struct tcp_fin_msg fin_msg;
    struct tcp_send_fin_msg send_fin_masg;
} tcp_stream;
...
inline int AddEpollEvent(struct mtcp_epoll *ep, int queue_type, socket_map_t socket, uint32_t event);

inline void RaiseReadEvent(mtcp_manager_t mtcp, tcp_stream *stream);

inline void RaiseWriteEvent(mtcp_manager_t mtcp, tcp_stream *stream);

inline void RaiseCloseEvent(mtcp_manager_t mtcp, tcp_stream *stream);

inline void RaiseErrorEvent(mtcp_manager_t mtcp, tcp_stream *stream);

tcp_stream *CreateTCPStream(mtcp_manager_t mtcp, socket_map_t socket, int type, uint32_t saddr, uint16_t sport, uint32_t daddr, uint16_t dport);
tcp_stream *CreateTCPStream_by_port(int type, uint32_t init_seq, 
		uint32_t saddr, uint16_t sport, uint32_t daddr, uint16_t dport);

void DestroyTCPStream(mtcp_manager_t mtcp, tcp_stream *stream);
void TOE_DestroyTCPStream(tcp_stream *stream);

void DumpStream(mtcp_manager_t mtcp, tcp_stream *stream);

//extern inline void InitializeTCPStreamManager(void);

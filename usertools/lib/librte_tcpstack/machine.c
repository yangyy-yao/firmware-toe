...
void reset_state(struct ccp_priv_state *state) {
    ...
    if (program == NULL) {
        TRACE_DEBUG("Cannot reset state because program is NULL\n");
    return;
    }
...
void init_register_state(struct ccp_priv_state *state) {
    ...
    struct DatapathProgram* program = datapath_program_lookup(state->program_index);
    if (program == NULL) {
        TRACE_DEBUG("Cannot init register state because program is NULL\n");
    return;
    }
...
void print_register(struct Register* reg) {
    ...
    TRACE_DEBUG("Register{%s(%u), ind: %d, val: %" PRIu64 "}\n", type, reg->type, reg->index, reg->value);
}
#endif
...
int process_instruction(int instr_index, struct ccp_priv_state *state, struct ccp_primitives* primitives) {
    ...
    switch (current_instruction.op) {
        case ADD:
            TRACE_DEBUG("ADD  %" PRIu64 " + %" PRIu64 " = %" PRIu64 "\n", arg1, arg2, myadd64(arg1, arg2)); 
            result = myadd64(arg1, arg2);
            if (result < arg1) {
                TRACE_DEBUG("ERROR! Integer overflow: %" PRIu64 " + %" PRIu64 "\n", arg1, arg2);
                return -1;
            }
            write_reg(state, result, current_instruction.rRet);
            break;
        case DIV:
            TRACE_DEBUG("DIV  %" PRIu64 " / %" PRIu64 " = ", arg1, arg2);
            if (arg2 == 0) {
                TRACE_DEBUG("ERROR! Attempt to divide by 0: %" PRIu64 " / %" PRIu64 "\n", arg1, arg2);
                return -1;
            } else {
                TRACE_DEBUG("%" PRIu64 "\n", mydiv64(arg1, arg2));
            ...
            break;
        case EQUIV:
            TRACE_DEBUG("EQV  %" PRIu64 " == %" PRIu64 " => %" PRIu64 "\n", arg1, arg2, myequiv64(arg1, arg2));
        ...
        case BIND: // take arg2, and put it in rRet
            TRACE_DEBUG("BIND r%" PRIu64 " -> r%" PRIu64 "\n", arg2, current_instruction.rRet.value);
            write_reg(state, arg2, current_instruction.rRet);
            break;
        default:
            TRACE_DEBUG("UNKNOWN OP %d\n", current_instruction.op);
            break;
    }
    return 0;
...
int state_machine(struct ccp_connection *conn) {
    ...
    if (state == NULL) {
        TRACE_DEBUG("CCP priv state is null");
        return -1;
    }
    struct DatapathProgram* program = datapath_program_lookup(state->program_index);
    if (program == NULL) {
        TRACE_DEBUG("Datapath program is null");
        return -1;
    }
    ...
    TRACE_DEBUG(">>> program starting [sid=%d] <<<\n", conn->index);
    ...
    for (i=0; i < program->num_expressions; i++) {
        ...
        if (ret < 0) {
            TRACE_DEBUG(">>> program finished [sid=%d] [ret=-1] <<<\n\n", conn->index);
            return -1;
        }
        ...
        TRACE_DEBUG("[sid=%d] fallthrough...\n", conn->index);
    }
    // set rate and cwnd from implicit registers
    if (state->registers.impl_registers[CWND_REG] > 0) {
        TRACE_DEBUG("[sid=%d] setting cwnd after program: %u\n", conn->index, state->registers.impl_registers[CWND_REG]);
        datapath->set_cwnd(datapath, conn, state->registers.impl_registers[CWND_REG]);
    }

    if (state->registers.impl_registers[RATE_REG] != 0) {
        TRACE_DEBUG("[sid=%d] setting rate after program: %u\n", conn->index, state->registers.impl_registers[CWND_REG]);
        datapath->set_rate_abs(datapath, conn, state->registers.impl_registers[RATE_REG]);
    }
    ...
    TRACE_DEBUG(">>> program finished [sid=%d] [ret=0] <<<\n\n", conn->index);
    return 0;
}

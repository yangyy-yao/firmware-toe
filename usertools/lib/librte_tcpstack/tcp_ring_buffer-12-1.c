...
struct tcp_ring_buffer* 
RBInit_no_copy(rb_manager_t rbm, uint32_t init_seq)
{
    struct tcp_ring_buffer* buff = (struct tcp_ring_buffer*)rte_zmalloc(NULL, sizeof(struct tcp_ring_buffer), RTE_CACHE_LINE_SIZE);
...
RBPut_no_copy(rb_manager_t rbm, struct tcp_ring_buffer* buff, 
       void* data, uint32_t len, uint32_t cur_seq, struct rte_mbuf *m)
{
    int putx, end_off;
    struct fragment_ctx *new_ctx;
    struct fragment_ctx* iter;
    ...
    */
	...
	printf("%s-%d: buffer queue is full! buff->data_buf.tail:%d\n",__func__,__LINE__,buff->data_buf.tail);
	return -2;
    }

	printf("%s-%d: mbuf:%p put queue, buff->data_buf.tail:%d\n",__func__,__LINE__,m,buff->data_buf.tail);
...
*/
    ...
    printf("%s-%d: new_ctx:%p, new_ctx->len:%d,new_ctx->seq:%u\n",__func__,__LINE__,new_ctx,new_ctx->len,new_ctx->seq);

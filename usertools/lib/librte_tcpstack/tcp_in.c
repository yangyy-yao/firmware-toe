        ...
        const struct tcphdr *tcph, uint32_t seq, uint16_t window)
{
    ...
    if (!cur_stream) {
        TRACE_DEBUG("INFO: Could not allocate tcp_stream!\n");
        return FALSE;
    }
        ...
        struct tcphdr *tcph, uint32_t seq, uint32_t ack_seq, int payloadlen)
{
    ...
    if (!tcph->rst && cur_stream->saw_timestamp) {
        ...
        if (!ParseTCPTimestamp(cur_stream, &ts, 
            ...
            /* TODO: implement here */
            TRACE_DEBUG("No timestamp found.\n");
            return FALSE;
        }
        ...
        if (TCP_SEQ_LT(ts.ts_val, cur_stream->rcvvar->ts_recent)) {
            ...
            TRACE_DEBUG("PAWS Detect wrong timestamp. "
            ...
            if (TCP_SEQ_GT(ts.ts_val, cur_stream->rcvvar->ts_recent)) {
                TRACE_DEBUG("Timestamp update. cur: %u, prior: %u "
                    "(time diff: %uus)\n", 
    ...
    if (!TCP_SEQ_BETWEEN(seq + payloadlen, cur_stream->rcv_nxt, 
        ...
        if (cur_stream->state == TCP_ST_ESTABLISHED) {
...
#if 0
                TRACE_DEBUG("Window update request. (seq: %u, rcv_wnd: %u)\n", 
            ...
            if (cur_stream->state == TCP_ST_TIME_WAIT) {
                TRACE_DEBUG("Stream %d: tw expire update to %u\n", 
...
{
    TRACE_DEBUG("Stream %d: Notifying connection reset.\n", cur_stream->id);
...
ProcessRST(mtcp_manager_t mtcp, tcp_stream *cur_stream, uint32_t ack_seq)
{
    ...
    TRACE_DEBUG("Stream %d: TCP RESET (%s)\n", 
...
#define TCP_RTO_MIN 0
    ...
    TRACE_DEBUG("mrtt: %u (%uus), srtt: %u (%ums), mdev: %u, mdev_max: %u, "
            "rttvar: %u, rtt_seq: %u\n", mrtt, mrtt * TIME_TICK, 
        ...
        uint16_t window, int payloadlen)
{
    ...
    if (TCP_SEQ_GT(ack_seq, sndvar->sndbuf->head_seq + sndvar->sndbuf->len)) {
        TRACE_DEBUG("Stream %d (%s): invalid acknologement. "
    ...
    if (TCP_SEQ_LT(cur_stream->rcvvar->snd_wl1, seq) ||
        ...
        if (cwindow_prev < cur_stream->snd_nxt - sndvar->snd_una && 
            ...
            TRACE_DEBUG("%u Broadcasting client window update! "
    ...
    if (!dup) {
        ...
        if (cur_stream->rcvvar->dup_acks >= 3) {
            TRACE_DEBUG("passed dup_acks, ack=%u, snd_nxt=%u, last_ack=%u len=%u wl2=%u peer_wnd=%u right=%u\n",
    ...
    if(cur_stream->wait_for_acks) {
        TRACE_DEBUG("got ack, but waiting to send... ack=%u, snd_next=%u cwnd=%u\n",
    ...
    if (dup && cur_stream->rcvvar->dup_acks == 3) {
        TRACE_DEBUG("Triple duplicated ACKs!! ack_seq: %u\n", ack_seq);
        TRACE_DEBUG("tridup ack %u (%u)!\n", ack_seq - cur_stream->sndvar->iss, ack_seq);
        if (TCP_SEQ_LT(ack_seq, cur_stream->snd_nxt)) {
            TRACE_DEBUG("Reducing snd_nxt from %u to %u\n",
            ...
            if (ack_seq != sndvar->snd_una) {
                TRACE_DEBUG("ack_seq and snd_una mismatch on tdp ack. "
                        "ack_seq: %u, snd_una: %u\n", 
...
#else
	    //printf("%s-%d:set snd_nxt cur_stream->snd_nxt:%u = ack_seq:%u\n",__func__,__LINE__,cur_stream->snd_nxt,ack_seq);
	    cur_stream->snd_nxt = ack_seq;
	    //printf("%s-%d: set sndvar->tcp_data_ring.head:%u = sndvar->tcp_data_ring.una_head:%u\n",__func__,__LINE__,sndvar->tcp_data_ring.head,sndvar->tcp_data_ring.una_head);
	    sndvar->tcp_data_ring.head = sndvar->tcp_data_ring.una_head;
#endif
        }
        ...
        TRACE_DEBUG("fast retrans: cwnd = ssthresh(%u)+3*mss = %u\n",
        ...
        if (sndvar->nrtx < TCP_MAX_RTX) {
            ...
            TRACE_DEBUG("Exceed MAX_RTX.\n");
        }

	//printf("%s-%d: AddtoSendList\n",__func__,__LINE__);
        ...
        if ((uint32_t)(sndvar->cwnd + sndvar->mss) > sndvar->cwnd) {
            sndvar->cwnd += sndvar->mss;
            TRACE_DEBUG("Dupack cwnd inflate. cwnd: %u, ssthresh: %u\n", sndvar->cwnd, sndvar->ssthresh);
        }
    }
...
#endif /* USE_CCP */
    {
        ...
        TRACE_DEBUG("Updating snd_nxt from %u to %u\n", cur_stream->snd_nxt, ack_seq);
#ifdef ENABLE_CCP
        cur_stream->wait_for_acks = FALSE;
#endif
	    //printf("%s-%d:set snd_next cur_stream->snd_nxt:%u = ack_seq:%u\n",__func__,__LINE__,cur_stream->snd_nxt,ack_seq);
        cur_stream->snd_nxt = ack_seq;
	set_snd_next = 1;
        TRACE_DEBUG("Sending again..., ack_seq=%u sndlen=%u cwnd=%u\n",
                        ack_seq-sndvar->iss,
                        sndvar->sndbuf->len,
                        sndvar->cwnd / sndvar->mss);
    ...
    if (rmlen > 0) {
        ...
        if (cur_stream->saw_timestamp) {
           ...
           //printf("%s-%d: sndvar->rto:%u,cur_stream->rcvvar->srtt:%u,cur_stream->rcvvar->rttvar:%u",__func__,__LINE__,sndvar->rto,cur_stream->rcvvar->srtt,cur_stream->rcvvar->rttvar);
	    TRACE_DEBUG("assert(sndvar->rto > 0)\n");
            assert(sndvar->rto > 0);
        } else {
        ...
        if (cur_stream->state >= TCP_ST_ESTABLISHED) {
            if (sndvar->cwnd < sndvar->ssthresh) {
                ...
                TRACE_DEBUG("slow start cwnd: %u, ssthresh: %u\n", 
                        sndvar->cwnd, sndvar->ssthresh);
            } else {
	...
	//printf("%s-%d:rmlen:%u,sndvar->snd_wnd:%u,sndvar->sndbuf->size:%u,sndvar->sndbuf->len:%u\n",__func__,__LINE__,rmlen,sndvar->snd_wnd,sndvar->sndbuf->size,sndvar->sndbuf->len);
	//printf("%s-%d: SBRemove before sndvar->tcp_data_ring.head:%u, sndvar->tcp_data_ring.una_head:%u\n",__func__,__LINE__,sndvar->tcp_data_ring.head,sndvar->tcp_data_ring.una_head);

	ret = SBRemove_no_copy(mtcp->rbm_snd, sndvar->sndbuf, &sndvar->tcp_data_ring, rmlen);
        sndvar->snd_una = ack_seq;
        snd_wnd_prev = sndvar->snd_wnd;
        sndvar->snd_wnd = sndvar->sndbuf->size - sndvar->sndbuf->len;
	if (sndvar->snd_wnd > snd_wnd_prev) {
	    sndvar->increment_wnd += (sndvar->snd_wnd - snd_wnd_prev);
	}
	printf("%s-%d:cur_stream:%p, sndvar:%p,sndvar->increment_wnd:%u \n",__func__,__LINE__,cur_stream, sndvar, sndvar->increment_wnd);
//	printf("%s-%d: SBRemove after sndvar->tcp_data_ring.head:%u, sndvar->tcp_data_ring.una_head:%u\n",__func__,__LINE__,sndvar->tcp_data_ring.head,sndvar->tcp_data_ring.una_head);

	//printf("%s-%d:sndvar->snd_una:%u,sndvar->snd_wnd:%u,sndvar->sndbuf->size:%u,sndvar->sndbuf->len:%u\n",__func__,__LINE__,sndvar->snd_una,sndvar->snd_wnd,sndvar->sndbuf->size,sndvar->sndbuf->len);
        ...
        uint32_t cur_ts, uint8_t *payload, uint32_t seq, int payloadlen)
{
    ...
    if (!rcvvar->rcvbuf) {
        ...
        if (!rcvvar->rcvbuf) {
            TRACE_DEBUG("Stream %d: Failed to allocate receive buffer.\n", cur_stream->id);
            cur_stream->state = TCP_ST_CLOSED;
    ...
    if (ret < 0) {
        TRACE_DEBUG("Cannot merge payload. reason: %d\n", ret);
    }
    ...
    TRACE_DEBUG("Stream %d data arrived. "
        ...
        int payloadlen, uint16_t window)
{
    ...
    if (tcph->syn && !tcph->ack) {
        ...
        if (!ret) {
            TRACE_DEBUG("Refusing SYN packet.\n");
#ifdef DBGMSG
        ...
        if (!cur_stream) {
            TRACE_DEBUG("Not available space in flow pool.\n");
#ifdef DBGMSG
            DumpIPPacket(mtcp, iph, ip_len);
#endif
            SendTCPPacketStandalone(mtcp, 
        ...
        TRACE_DEBUG("Reset packet comes\n");
#ifdef DBGMSG
        DumpIPPacket(mtcp, iph, ip_len);
#endif
        ...
        TRACE_DEBUG("Weird packet comes.\n");
#ifdef DBGMSG
        DumpIPPacket(mtcp, iph, ip_len);
#endif
...
Handle_TCP_ST_LISTEN (mtcp_manager_t mtcp, uint32_t cur_ts, 
    ...
    if (tcph->syn) {
        ...
        TRACE_DEBUG("Stream %d: TCP_ST_SYN_RCVD\n", cur_stream->id);
        AddtoControlList(mtcp, cur_stream, cur_ts);
    } else {
        TRACE_DEBUG("Stream %d (TCP_ST_LISTEN): Packet without SYN.\n", cur_stream->id);
    }
        ...
        uint32_t seq, uint32_t ack_seq, int payloadlen, uint16_t window)
{
    ...
    if (tcph->syn) {
        if (tcph->ack) {
            ...
            TRACE_DEBUG("Stream %d: TCP_ST_ESTABLISHED\n", cur_stream->id);

            if (cur_stream->socket) {
                RaiseWriteEvent(mtcp, cur_stream);
            } else {
                TRACE_DEBUG("Stream %d: ESTABLISHED, but no socket\n", cur_stream->id);
                SendTCPPacketStandalone(mtcp, 
                        iph->daddr, tcph->dest, iph->saddr, tcph->source, 
                        0, seq + payloadlen + 1, 0, TCP_FLAG_RST | TCP_FLAG_ACK, 
                        NULL, 0, cur_ts, 0);
        ...
        tcp_stream* cur_stream, struct tcphdr* tcph, uint32_t ack_seq) 
{
    ...
    if (tcph->ack) {
        ...
        TRACE_DEBUG("sync_recvd: updating cwnd from %u to %u\n", prior_cwnd, sndvar->cwnd);
        ...
        if (ret < 0) {
            TRACE_DEBUG("Stream %d: Failed to enqueue to "
                    "the listen backlog!\n", cur_stream->id);
            cur_stream->close_reason = TCP_NOT_ACCEPTED;
            cur_stream->state = TCP_ST_CLOSED;
            TRACE_DEBUG("Stream %d: TCP_ST_CLOSED\n", cur_stream->id);
            AddtoControlList(mtcp, cur_stream, cur_ts);
        }
        //TRACE_DEBUG("Stream %d inserted into acceptq.\n", cur_stream->id);
        if (CONFIG.tcp_timeout > 0)
            AddtoTimeoutList(mtcp, cur_stream);
        ...
        TRACE_DEBUG("Stream %d (TCP_ST_SYN_RCVD): No ACK.\n", 
                cur_stream->id);
        /* retransmit SYN/ACK */
        cur_stream->snd_nxt = sndvar->iss;
        AddtoControlList(mtcp, cur_stream, cur_ts);
...
{
    if (tcph->syn) {
        TRACE_DEBUG("Stream %d (TCP_ST_ESTABLISHED): weird SYN. "
    ...
    if (tcph->fin) {
        ...
        if (seq + payloadlen == cur_stream->rcv_nxt) {
            cur_stream->state = TCP_ST_CLOSE_WAIT;
            TRACE_DEBUG("Stream %d: TCP_ST_CLOSE_WAIT\n", cur_stream->id);
            cur_stream->rcv_nxt++;
            AddtoControlList(mtcp, cur_stream, cur_ts);
        ...
        uint32_t seq, uint32_t ack_seq, int payloadlen, uint16_t window) 
{
    ...
    if (TCP_SEQ_LT(seq, cur_stream->rcv_nxt)) {
        TRACE_DEBUG("Stream %d (TCP_ST_LAST_ACK): "
                "weird seq: %u, expected: %u\n", 
                cur_stream->id, seq, cur_stream->rcv_nxt);
        return;
    }
    ...
    if (tcph->ack) {
        ...
        if (ack_seq == cur_stream->sndvar->fss + 1) {
            ...
            TRACE_DEBUG("Stream %d (TCP_ST_LAST_ACK): Not ACK of FIN. "
        ...
        TRACE_DEBUG("Stream %d (TCP_ST_LAST_ACK): No ACK\n", cur_stream->id);
        //cur_stream->snd_nxt = cur_stream->sndvar->fss;
        AddtoControlList(mtcp, cur_stream, cur_ts);
    }
}
        ...
        uint8_t *payload, int payloadlen, uint16_t window) 
{
    ...
    if (tcph->ack) {
        ...
        if (cur_stream->sndvar->is_fin_sent && 
            ...
            if (TCP_SEQ_GT(ack_seq, cur_stream->snd_nxt)) {
                TRACE_DEBUG("Stream %d: update snd_nxt to %u\n", 
            ...
            RemoveFromRTOList(mtcp, cur_stream);
            cur_stream->state = TCP_ST_FIN_WAIT_2;
            TRACE_DEBUG("Stream %d: TCP_ST_FIN_WAIT_2\n", cur_stream->id);
        }

    } else {
        TRACE_DEBUG("Stream %d: does not contain an ack!\n", 
                cur_stream->id);
        return;
    }
    ...
    if (tcph->fin) {
        ...
        if (seq + payloadlen == cur_stream->rcv_nxt) {
            ...
            if (cur_stream->state == TCP_ST_FIN_WAIT_1) {
                cur_stream->state = TCP_ST_CLOSING;
                TRACE_DEBUG("Stream %d: TCP_ST_CLOSING\n", cur_stream->id);

            } else if (cur_stream->state == TCP_ST_FIN_WAIT_2) {
                cur_stream->state = TCP_ST_TIME_WAIT;
                TRACE_DEBUG("Stream %d: TCP_ST_TIME_WAIT\n", cur_stream->id);
                AddtoTimewaitList(mtcp, cur_stream, cur_ts);
            }
            AddtoControlList(mtcp, cur_stream, cur_ts);
        }
        ...
        uint8_t *payload, int payloadlen, uint16_t window)
{
    ...
    if (tcph->fin) {
        ...
        TRACE_DEBUG("Stream %d (TCP_ST_FIN_WAIT_2): No FIN. "
...
Handle_TCP_ST_CLOSING (mtcp_manager_t mtcp, uint32_t cur_ts, 
    ...
    if (tcph->ack) {
        ...
        TRACE_DEBUG("Stream %d (TCP_ST_CLOSING): Not ACK\n", cur_stream->id);
        return;
    }
}
/*----------------------------------------------------------------------------*/
         ...
         uint32_t cur_ts, const int ifidx, const struct iphdr *iph, int ip_len)
{
    ...
    if (rc == -1) {
        ...
        if (check) {
            TRACE_DEBUG("Checksum Error: Original: 0x%04x, calculated: 0x%04x\n", 
    ...
    if (cur_stream->state > TCP_ST_SYN_RCVD) {
        ...
        if (!ret) {
            TRACE_DEBUG("Stream %d: Unexpected sequence: %u, expected: %u\n",
                    cur_stream->id, seq, cur_stream->rcv_nxt);
#ifdef DBGMSG
            DumpIPPacket(mtcp, iph, ip_len);
#endif

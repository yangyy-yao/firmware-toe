...
GetOutputInterface(uint32_t daddr, uint8_t *is_external)
{
    ...
    *is_external = 0;
    ...
    if (nif < 0) {
        ...
        TRACE_DEBUG("[WARNING] No route to %u.%u.%u.%u\n", da[0], da[1], da[2], da[3]);
        assert(0);
    }
        ...
        uint16_t ip_id, uint32_t saddr, uint32_t daddr, uint16_t payloadlen)
{
    ...
    haddr = NULL;//GetDestinationHWaddr(daddr, is_external);
    if (!haddr) {
#if 0
        uint8_t *da = (uint8_t *)&daddr;
    ...
    iph = (struct iphdr *)EthernetOutput(mtcp, 
            ETH_P_IP, nif, haddr, payloadlen + IP_HEADER_LEN, NULL, payloadlen, 0);
    if (!iph) {
        return NULL;
    }
...
/*----------------------------------------------------------------------------*/
uint8_t *
IPOutput(struct mtcp_manager *mtcp, tcp_stream *stream, uint16_t tcplen, uint16_t payloadlen, int data_posit)
{
    struct iphdr *iph;
    int nif;
    unsigned char *haddr = NULL, is_external = 0;
    int rc = -1;
/*
...
*/
    ...
    iph = (struct iphdr *)EthernetOutput(mtcp, ETH_P_IP, 
            stream->sndvar->nif_out, haddr, tcplen + IP_HEADER_LEN, stream, payloadlen, data_posit);
    if (!iph) {
        return NULL;
    }

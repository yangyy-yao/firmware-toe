    ...
    for (i = 0; i < len; ) {
        ...
        if (opt == TCP_OPT_END) {   // end of option field
            ...
            if (opt == TCP_OPT_MSS) {
                ...
                TRACE_DEBUG("Remote SACK permited.\n");
            } else if (opt == TCP_OPT_TIMESTAMP) {
                TRACE_DEBUG("Saw peer timestamp!\n");
...
#endif
                        TRACE_DEBUG("SACK entry. left_edge: %u, right_edge: %u (ack_seq: %u)\n", left_edge, right_edge, ack_seq);

                    }
                    TRACE_DEBUG("Found SACK entry. left_edge: %u, right_edge: %u\n", left_edge, right_edge);
                }
                i += j;
            } else {
                // not handle

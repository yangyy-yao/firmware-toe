...
struct rte_flow *
generate_tcp_flow(uint16_t port_id, uint16_t rx_q, int core)
{
	struct rte_flow_attr attr;
	struct rte_flow_item pattern[2];
	...
	printf("%s-%d: rx_q:%d, core:%d, spec:%u\n",__func__,__LINE__,rx_q,core,spec);
...
//		}
		...
		for(queue = 0; queue < qconf->n_rx_queue; ++queue) {
			...
			if (ret < 0)
				...
				ret, portid);
			if (portid == 0 && !queueid) {
				if (eth_flow_generate(portid) == NULL)
					rte_exit(EXIT_FAILURE, "eth_flow_generate failed:port=%d, core=%d\n",
														portid, lcore_id);
			}
			if (portid == 0) {
				if (generate_tcp_flow(portid, queueid, lcore_id) == NULL) {
					rte_exit(EXIT_FAILURE, "tcp_flow_generate:port=%d, core=%d\n", portid, lcore_id);
				}
			}
		}
	}

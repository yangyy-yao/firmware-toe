...
static void toe_rx_data_fill_cq_msg(struct toe_data_rx_rq_info *data_rq, uint16_t len, uint8_t result, struct toe_engine *toe_eg, struct toe_data_host_to_dpu_req *rq_msg, tcp_stream *stream, int qid)
{
	struct toe_data_rx_cq_info *cq_info = &toe_eg->data_rx_vring[qid]->cq_info;
	struct toe_data_host_to_dpu_res *cq_msg;
	uint16_t cq_rq_head;
	...
	cq_msg->sent_len = len;
	*/
	if (stream->sndvar->increment_wnd == 0 && stream->sndvar->snd_wnd == 0) {

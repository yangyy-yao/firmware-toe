																	...
																	int idx)
{
	...
	if (tcp_s && (unlikely(tcp_s->connect_state == TOE_CLOSED) || unlikely(tcp_s->connect_state == TOE_CLOSING))) {
	...
	//printf("%s-%d:idx:%d, send_ctlpkt:%d,tcp ctrl flags:0x%x,tcp_s->connect_state:%d!\n",__func__,__LINE__,idx,send_ctlpkt,flags,tcp_s->connect_state);
	if (send_ctlpkt) {
...
uint16_t toe_rx_pkt_burst(void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t nb_pkts)
{
	...
	const uint64_t drain_tsc = (rte_get_tsc_hz() + TOE_US_PER_S - 1) / TOE_US_PER_S * TOE_BURST_TX_DRAIN_US;

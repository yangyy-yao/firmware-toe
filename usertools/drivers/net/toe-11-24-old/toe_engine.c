...
//		struct rte_mbuf *mbuf;
		...
		if (tcp_s == NULL) {
			tcp_s = CreateTCPStream_by_port(0, acr->tcp_params.sequence, acr->local_ip, acr->local_port, acr->remote_ip, acr->remote_port);
			if (!tcp_s) {
				return -1;
			}
		...
		tcp_s->snd_nxt = acr->tcp_params.sequence;
		tcp_s->rcvvar->rcv_wnd = acr->tcp_params.window;
		tcp_s->sndvar->wscale_mine = acr->tcp_params.window_scale;
		tcp_s->host_dataptr = rq_msg->id.host_dataptr;
		printf("%s-%d,tcp_s:%p,tcp_s->state:%d,tcp_s->host_dataptr:%llx,acr->tcp_params.sequence:%d \n", __func__, __LINE__,tcp_s,tcp_s->state,tcp_s->host_dataptr,acr->tcp_params.sequence);
...
{
		tcp_stream *tcp_s;
		struct toe_h2d_msg_active_connect_confirm_hdr *acc = &rq_msg->active_connect_confirm;
		int ret;
		...
		tcp_s->snd_nxt = acc->tcp_params.sequence;
		tcp_s->rcv_nxt = acc->tcp_params.acknowledge;
		tcp_s->rcvvar->rcv_wnd = acc->tcp_params.window;
		tcp_s->sndvar->wscale_mine = acc->tcp_params.window_scale;
		ret = toe_send_tcppkt_to_ring(tcp_s, TCP_FLAG_ACK, NULL, 0);
		if (ret == 0)
			tcp_s->state = TCP_ST_ESTABLISHED;

		toe_cq_common_msg(rq_msg, cq_msg, toe_eg, tcp_s, idx);
        printf("%s-%d: !!! stream->sndvar->tcp_data_ring.tail=%u  free_num=%u\n",__func__, __LINE__, tcp_s->sndvar->tcp_data_ring.tail, tcp_s->sndvar->tcp_data_ring.free_num);
...
{
		tcp_stream *tcp_s;
		struct toe_h2d_msg_passive_connect_accept_hdr *pca = &rq_msg->passive_connect_accept;
		int ret;
        printf("%s:%d \n", __func__, __LINE__);
		...
		tcp_s->host_dataptr = rq_msg->id.host_dataptr;
		tcp_s->sndvar->iss = pca->tcp_params.sequence;
		tcp_s->snd_nxt = pca->tcp_params.sequence;
		tcp_s->rcv_nxt = pca->tcp_params.acknowledge;
		tcp_s->rcvvar->rcv_wnd = pca->tcp_params.window;
		tcp_s->sndvar->wscale_mine = pca->tcp_params.window_scale;
																	...
																	int idx)
{
		...
		tcp_s->state = TCP_ST_FIN_WAIT_1;
		sndvar = tcp_s->sndvar;

		if (sndvar->sndbuf) {
			sndvar->fss = sndvar->sndbuf->head_seq + sndvar->sndbuf->len;
		} else {
			sndvar->fss = tcp_s->snd_nxt;
		}
		printf("%s-%d: sndvar->fss:%d\n",__func__,__LINE__,sndvar->fss);
...
{
		tcp_stream *tcp_s;
		struct toe_h2d_msg_active_close_complete_hdr *acc = &rq_msg->active_close_complete;
		int ret;
        printf("%s:%d \n", __func__, __LINE__);
		...
		tcp_s->host_dataptr = rq_msg->id.host_dataptr;
		//tcp_s->sndvar->iss = acc->tcp_params.sequence;
		tcp_s->snd_nxt = acc->tcp_params.sequence;
		tcp_s->rcv_nxt = acc->tcp_params.acknowledge;
		//tcp_s->rcvvar->rcv_wnd = acc->tcp_params.window;
		//tcp_s->sndvar->wscale_mine = acc->tcp_params.window_scale;
...
int toe_dma_data_to_host(tcp_stream *stream, struct toe_engine *toe_eg, int qid, int timeout_send)
{
		...
		printf("%s-%d: merged_len:%d, rcvvar->rcvbuf->data_buf.prev_head:%d,rcvvar->rcvbuf->data_buf.tail:%d, rq->recv_buffer[rq->use_idx].recv_buffer_len:%d\n",__func__,__LINE__,merged_len, rcvvar->rcvbuf->data_buf.prev_head,rcvvar->rcvbuf->data_buf.tail,rq->recv_buffer[rq->use_idx].recv_buffer_len);
		while (merged_len && (merged_len > rq->recv_buffer[rq->use_idx].recv_buffer_len || timeout_send)) {
			...
			do { 
				...
				printf("%s-%d,m:%p,en_len:%d,dma_final_len:%d,en_len_total:%d,merged_len:%d,head:%d,h_buffer_addr:0x%llx,rq->use_idx\n",__func__,__LINE__,m,m?m->data_len:0,dma_final_len,en_len_total,merged_len,head,h_buffer_addr,rq->use_idx);
				en_len = m->data_len;
			...
			if (rq->use_idx + 1 == TOE_RECV_BUFFER_COUNT_RESERVED) { //rq中所有buffer写完，换下一个rq
...
//	uint16_t window = ntohs(tcph->rx_win);
	...
	if (TCP_SEQ_LEQ(stream->rcv_nxt, prev_rcv_nxt)) {
		...
		printf("%s-%d: There are some lost packets rcv_nxt:%d,prev_rcv_nxt\n",__func__,__LINE__,stream->rcv_nxt,prev_rcv_nxt);
		return FALSE;
	}
...
int toe_tcp_ctlpkt_to_host(struct rte_mbuf *m, struct rte_tcp_hdr *tcph, tcp_stream *stream, struct toe_engine *toe_eg, int qid)
{
	...
	switch (flag) {
		case TCP_FLAG_SYN:
				...
				cq_msg->passive_connect_request.tcp_params.acknowledge = ntohl(tcph->recv_ack);
		...
		case (TCP_FLAG_SYN | TCP_FLAG_ACK):
				...
				cq_msg->active_connect_accept.tcp_params.sequence = ntohl(tcph->sent_seq);
				cq_msg->active_connect_accept.tcp_params.acknowledge = ntohl(tcph->recv_ack);
				cq_msg->active_connect_accept.tcp_params.window = ntohl(tcph->rx_win);
		...
		case (TCP_FLAG_FIN | TCP_FLAG_ACK):
				...
				} else if (stream->state == TCP_ST_FIN_WAIT_2) {
					cq_msg->opcode = TOE_MSG_OPCODE_D2H_ACTIVE_CLOSE_CONFIRM;
					cq_msg->active_close_confirm.tcp_params.sequence = ntohl(tcph->sent_seq);
					cq_msg->active_close_confirm.tcp_params.acknowledge = ntohl(tcph->recv_ack);
					cq_msg->active_close_confirm.tcp_params.window = ntohl(tcph->rx_win);
				}
				stream->sndvar->snd_una++;
				printf("%s-%d:stream->sndvar->snd_una:%d,\n",__func__,__LINE__,stream->sndvar->snd_una);
		...
		case TCP_FLAG_ACK:
				if (stream->state == TCP_ST_SYN_RCVD) {
					...
					cq_msg->passive_connect_confirm.tcp_params.sequence = ntohl(tcph->sent_seq);
					cq_msg->passive_connect_confirm.tcp_params.acknowledge = ntohl(tcph->recv_ack);
					cq_msg->passive_connect_confirm.tcp_params.window = ntohl(tcph->rx_win);
					stream->state = TCP_ST_ESTABLISHED;
				} else if (stream->state == TCP_ST_FIN_WAIT_1) {
					cq_msg->opcode = TOE_MSG_OPCODE_D2H_ACTIVE_CLOSE_ACCEPT;
					cq_msg->active_close_accept.tcp_params.sequence = ntohl(tcph->sent_seq);
					cq_msg->active_close_accept.tcp_params.acknowledge = ntohl(tcph->recv_ack);
					cq_msg->active_close_accept.tcp_params.window = ntohl(tcph->rx_win);
					stream->state = TCP_ST_FIN_WAIT_2;
...
data_deal:
	...
	if (payloadlen > 0) {
		...
		toe_dma_data_to_host(stream, toe_eg, qid, 1);
	}

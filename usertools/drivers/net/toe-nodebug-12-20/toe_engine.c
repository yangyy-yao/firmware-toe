																	...
																	int idx)
{
	...
	if (tcp_s && tcp_s->connect_state == TOE_CLOSED) {
...
static int toe_tcp_fin_to_host(tcp_stream *stream, struct toe_engine *toe_eg, int qid)
{
	...
	cq_msg->ctrl_pkt_msg.tcp_params.tcp_flags = stream->fin_msg.tcp_flag;
	cq_msg->ctrl_pkt_msg.tcp_params.sequence = ntohl(stream->fin_msg.sequence);
	cq_msg->ctrl_pkt_msg.tcp_params.window = ntohl(stream->fin_msg.window);
	cq_msg->ctrl_pkt_msg.tcp_params.acknowledge = ntohl(stream->fin_msg.acknowledge);

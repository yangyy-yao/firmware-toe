...
struct toe_data_recv_buffer {
    unsigned long long          recv_buffer_phyaddr;
    unsigned short              recv_buffer_len;
    unsigned long               host_list_virtaddr;
}__attribute__((__packed__));
...
struct toe_data_host_to_dpu_res {  //channel 2 cq
		...
		unsigned char result;
		unsigned short rq_head;
		unsigned short sent_len;
        unsigned long send_list_virtaddr;
		struct toe_socket_id identification;
}__attribute__((__packed__));
...
struct toe_data_dpu_to_host_res{  //channel 3 cq
    ...
    unsigned short       data_len;
    struct toe_socket_id identification;
    unsigned long        recv_list_virtaddr;
}__attribute__((__packed__));

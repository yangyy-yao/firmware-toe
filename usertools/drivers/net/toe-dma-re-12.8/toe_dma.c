...
static void toe_rx_data_fill_cq_msg(uint16_t rq_head, uint16_t len, uint8_t result, struct toe_engine *toe_eg, struct toe_data_host_to_dpu_req *rq_msg, void *stream, int qid)
{
	...
	cq_msg->send_buffer_phyaddr = rq_msg->send_buffer_addr;
...
//	uint16_t site;
	...
	for (j = 0; j < num; j++) {
		...
		m_final_num = m_num;
		if (stream->sndvar->tcp_data_ring.free_num < m_final_num || stream->sndvar->snd_wnd == 0) {
			...
			toe_rx_data_fill_cq_msg(data_rq->head, 0, TOE_DMA_EAGAIN, toe_eg, data_msg, stream, idx);
		...
		for (i = 0; i < m_num; i++) {
			...
			*m_data_len = job_len;
			...
			if (i == m_num - 1) {
				...
				if (toe_sendbuf_update(stream, en_len) < 0) {
					printf("%s-%d: sendbuf update failed\n");
					data_get_failed = 1;
					count -= m_num;
					rte_mempool_put_bulk(dma->jpool, (void**)&sjob[0], m_num);
					...
					printf("%s-%d:en_len:%u,data_msg->data_len:%u\n",__func__,__LINE__,en_len,data_msg->data_len);
					if (en_len >= data_msg->data_len)
						toe_rx_data_fill_cq_msg(data_rq->head, en_len, TOE_DMA_SUCCESS, toe_eg, data_msg, stream, idx);
					else 
						toe_rx_data_fill_cq_msg(data_rq->head, en_len, TOE_DMA_EAGAIN, toe_eg, data_msg, stream, idx);
				}
			}

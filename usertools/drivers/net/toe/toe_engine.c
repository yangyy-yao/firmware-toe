...
int toe_send_tcppkt_to_ring(tcp_stream *tcp_s, uint8_t tcpflag, void *data, int data_len)
{
	...
	if (TOE_LOG_ON) {
		...
		if (log_count < LOG_NUM) {
				...
				sprintf(timestamp_str[log_count], "toe_send_tcppkt_to_ring"); 
				log_count ++;
		}
		else {
			timestamp[log_count] = (rte_rdtsc()*1000000)/rte_get_tsc_hz();                                
																	...
																	int idx)
{
	...
	if (TOE_LOG_ON) {
		...
		if (log_count < LOG_NUM) {
				timestamp[log_count] = (rte_rdtsc()*1000000)/rte_get_tsc_hz();
				sprintf(timestamp_str[log_count], "toe_h2d_ctrl_msg_0x%x", flags); 
				log_count ++;
		}
		else {
			timestamp[log_count] = (rte_rdtsc()*1000000)/rte_get_tsc_hz();                                
	...
	switch (flags) {
		...
		case (TCP_FLAG_RST | TCP_FLAG_ACK):
			if (tcp_s->connect_state != TOE_ESTABLISHED) {
	    		...
	    		printf("%s-%d: tcp_s->snd_nxt:%u,tcp_s->sndvar->snd_una:%u \n",__func__,__LINE__,tcp_s->snd_nxt, tcp_s->sndvar->snd_una);
		...
		case TCP_FLAG_ACK:
			...
			} else if (tcp_s->connect_state == TOE_CLOSED){
				if (TOE_LOG_ON) {
					int i;
					for (i = 0; i < log_count; i++){
						printf("time : %s %llu\n",timestamp_str[i], timestamp[i]);
					}
				}
			}
			break;
		case (TCP_FLAG_FIN | TCP_FLAG_ACK):
			...
			if (tcp_s->sndvar->tcp_data_ring.free_num < TCP_SEND_DATA_BUFFER_MAX_NUM) {
				...
				tcp_s->send_fin_masg.connect_state = ctrl_msg->status;
	    			printf("%s-%d: free_num:%u,tcp_s->snd_nxt:%u,tcp_s->sndvar->snd_una:%u \n",__func__,__LINE__,tcp_s->sndvar->tcp_data_ring.free_num,tcp_s->snd_nxt, tcp_s->sndvar->snd_una);
				break;
			}
			...
			if (tcp_s->connect_state == TOE_CLOSING) {
	    			printf("%s-%d: tcp_s->snd_nxt:%u,ctrl_msg->tcp_info.tcp_params.sequence:%u,tcp_s->sndvar->snd_una:%u \n",__func__,__LINE__,tcp_s->snd_nxt, ctrl_msg->tcp_info.tcp_params.sequence,tcp_s->sndvar->snd_una);
				tcp_s->snd_nxt = ctrl_msg->tcp_info.tcp_params.sequence;
				tcp_s->rcv_nxt = ctrl_msg->tcp_info.tcp_params.acknowledge;
			}
																		...
																		int idx)
{
		mtcp_manager_t mtcp = g_mtcp[rte_lcore_id()];
	    	...
	    	printf("%s-%d:stream->snd_nxt:%u,stream->sndvar->snd_una:%u \n",__func__,__LINE__,stream->snd_nxt, stream->sndvar->snd_una);
...
//				printf("%s-%d: no host buffer\n",__func__,__LINE__);
		...
		if (likely(free_len > 0)) {
			...
			if (stream->need_wnd_adv) {
				printf("%s-%d: stream->sndvar->eff_mss:%u\n",__func__,__LINE__,stream->sndvar->eff_mss);
...
int toe_dma_data_to_host(tcp_stream *stream, struct toe_engine *toe_eg, int qid, int timeout_send)
{
		...
		if (TOE_LOG_ON) {
			if (log_count < LOG_NUM) {
				timestamp[log_count] = (rte_rdtsc()*1000000)/rte_get_tsc_hz();
				sprintf(timestamp_str[log_count], "toe_dma_data_to_host"); 
				log_count ++;
			}
			else {
				timestamp[log_count] = (rte_rdtsc()*1000000)/rte_get_tsc_hz();                                
		...
		while (m && merged_len && (merged_len > rq->recv_buffer[rq->use_idx].recv_buffer_len || timeout_send)) {
			...
			if (rq->use_idx + 1 == rq->buffer_num) { //rq中所有buffer写完，换下一个rq
				data_rq->pre_head = (data_rq->pre_head + 1) % data_rq->rq_size;
				if (data_rq->pre_head == data_rq->real_tail) { //没有可用的rq
					printf("%s-%d: data_rq->real_tail:%u, no host buffer\n",__func__,__LINE__,data_rq->real_tail);
					break;
				}
				rq = data_rq->rq_local + data_rq->pre_head;
			} else {
...
static int toe_dma_data_to_host(tcp_stream *stream, struct toe_engine *toe_eg, int qid, int timeout_send)
{
		...
		//printf("%s-%d: merged_len:%d!,m:%p\n",__func__,__LINE__,merged_len,m);
		while (m && merged_len && (merged_len > rq->recv_buffer.recv_buffer_len || timeout_send)) {
			dma_final_len = 0;
			en_len_total = 0;
			host_list_addr = 0;
...
//				if (!m)
//					printf("%s-%d,prev_m:%p,prev_m->data_len:%d,m:%p,m->data_len:%d,dma_final_len:%d,en_len_total:%d,merged_len:%d,h_buffer_addr:0x%llx\n",__func__,__LINE__, prev_m,prev_m->data_len, m,m?m->data_len:-1,dma_final_len,en_len_total,merged_len,h_buffer_addr);		
				jobs[jobs_num] = toe_tx_databuf_to_job(prev_m, h_buffer_addr, en_len, dma_final_len, stream->host_dataptr, host_list_addr,toe_eg,qid, stream, rec_ring->tail);
				if (!jobs[jobs_num]) {
					goto done;
				}
			...
			//printf("%s-%d: merged_len:%u,dma_final_len:%u!\n",__func__,__LINE__,merged_len,dma_final_len);
			//rq中所有buffer写完，换下一个rq
			data_rq->pre_head = (data_rq->pre_head + 1) % data_rq->rq_size;
			if (data_rq->pre_head == data_rq->real_tail) { //没有可用的rq
				printf("%s-%d: no host buffer, data_rq->pre_head:%d,data_rq->real_tail:%d,data_rq->enq_tail:%d,*data_rq->tail:%d,loop_count:%llu,now:%llu\n",__func__,__LINE__,data_rq->pre_head,data_rq->real_tail,data_rq->enq_tail,*data_rq->tail,loop_count,(rte_rdtsc()*1000000)/rte_get_tsc_hz());
				break;
			}
			rq = data_rq->rq_local + data_rq->pre_head;
		...
		if (TOE_LOG_ON) {
			if (log_count < LOG_NUM) {
				timestamp[log_count] = (rte_rdtsc()*1000000)/rte_get_tsc_hz();
				sprintf(timestamp_str[log_count], "toe_dma_data_to_host-end"); 
				log_count ++;
			}
			else {
				timestamp[log_count] = (rte_rdtsc()*1000000)/rte_get_tsc_hz();                                
...
__rte_always_inline static uint32_t toe_get_inc_wnd_threshold(tcp_stream *stream)
{
	uint32_t sep = stream->sndvar->sndbuf->size / TOE_INC_WND_MAX_TAPS;

	return ((stream->sndvar->snd_wnd / sep) + 1) * TOE_INC_WND_PER_STEP;
}
...
int toe_process_tcp_payload(tcp_stream *stream, struct rte_tcp_hdr *tcph, int payloadlen, struct rte_mbuf *m, struct toe_engine *toe_eg)
{
	...
	rcvvar->rcv_wnd = rcvvar->rcvbuf->size - rcvvar->rcvbuf->merged_len;
/*	if (rcvvar->rcv_wnd < 100) {
		struct rte_mbuf *mbu;
		mbu = rcvvar->rcvbuf->fctx->head_mbuf;
		printf("%s-%d: rcvvar->rcvbuf->fctx->head_mbuf:%p, rcvvar->rcvbuf->fctx->tail_mbuf:%p,rcvvar->rcvbuf->fctx->len:%u,\n",__func__,__LINE__,rcvvar->rcvbuf->fctx->head_mbuf, rcvvar->rcvbuf->fctx->tail_mbuf,rcvvar->rcvbuf->fctx->len);
		while(mbu) {
			printf("%s-%d: mbu:%p, mbu->data_len:%u\n",__func__,__LINE__,mbu, mbu->data_len);
			mbu = mbu->next;
		}
	}
*/
	if (TCP_SEQ_LEQ(stream->rcv_nxt, prev_rcv_nxt)) {
		...
		printf("%s-%d: There are some lost packets rcv_nxt:%u,prev_rcv_nxt:%u,seq:%u\n",__func__,__LINE__,stream->rcv_nxt,prev_rcv_nxt,seq);
...
//	printf("%s-%d:m:%p,m->pkt_len:%d,nb_seg:%d,m->data_len:%d,seq:%u,rcvvar->rcvbuf->merged_len:%d\n", __func__,__LINE__,m,m->pkt_len,m->nb_segs,m->data_len, seq, rcvvar->rcvbuf->merged_len);
//	printf("%s-%d:done  loop_count:%llu,now:%llu \n",__func__,__LINE__,loop_count,(rte_rdtsc()*1000000)/rte_get_tsc_hz());
...
static int toe_pkt_parse_tcp(struct rte_mbuf *mbuf, struct toe_engine *toe_eg, int qid, uint32_t cur_ts)
{
	...
	s_stream.dport = tcph->src_port;
		//printf("%s-%d:qid:%d, saddr:0x%x,daddr:0x%x,sport:%d,dport:%d,tcp_flag:0x%x,payloadlen:%d,iph->total_length:%d,payload:%p,iph:%p\n",__func__,__LINE__,qid,ntohl(iph->src_addr), ntohl(iph->dst_addr), ntohs(tcph->src_port), ntohs(tcph->dst_port), tcp_flag,payloadlen,ntohs(iph->total_length),payload,(u_char *)iph);
		//printf("%s-%d:loop_count:%llu,now:%llu \n",__func__,__LINE__,loop_count,(rte_rdtsc()*1000000)/rte_get_tsc_hz());
	if (TOE_LOG_ON) {

		if (log_count < LOG_NUM) {
				timestamp[log_count] = (rte_rdtsc()*1000000)/rte_get_tsc_hz();
				sprintf(timestamp_str[log_count], "toe_pkt_parse_tcp-0x%x",tcp_flag); 
				log_count ++;
		}
		else {
			timestamp[log_count] = (rte_rdtsc()*1000000)/rte_get_tsc_hz();                                
...
dma_to_host:
	if (stream->connect_state == TOE_ESTABLISHING && tcp_flag == TCP_FLAG_ACK) {
		...
		if (ack_seq == stream->sndvar->iss + 1) {
			printf("%s-%d: connect_state set TOE_CLOSED! tcph->dst_port:%u\n",__func__,__LINE__, tcph->dst_port);
			stream->connect_state = TOE_CLOSED;
		}
	}
	...
	if (to_host) {
		...
		if (TOE_LOG_ON) {
			...
			if (log_count < LOG_NUM) {
						timestamp[log_count] = (rte_rdtsc()*1000000)/rte_get_tsc_hz();
						sprintf(timestamp_str[log_count], "toe_pkt_parse_tcp-end"); 
						log_count ++;
			}
			else {
				timestamp[log_count] = (rte_rdtsc()*1000000)/rte_get_tsc_hz();                                
...
data_deal:
	...
	if (payloadlen > 0) {
		...
		if (TOE_LOG_ON) {
			...
			if (log_count < LOG_NUM) {
				timestamp[log_count] = (rte_rdtsc()*1000000)/rte_get_tsc_hz();
				sprintf(timestamp_str[log_count], "toe_data_stream_enq"); 
				log_count ++;
			}
			else {
				timestamp[log_count] = (rte_rdtsc()*1000000)/rte_get_tsc_hz();                                
	...
	if (tcp_flag & TCP_FLAG_ACK) {
		if (likely(stream->sndvar->sndbuf)) {
			uint32_t inc_thresh = toe_get_inc_wnd_threshold(stream);
			printf("%s-%d:stream->sndvar->snd_wnd:%u, stream->sndvar->increment_wnd:%u\n",__func__,__LINE__,stream->sndvar->snd_wnd,stream->sndvar->increment_wnd);
			ProcessACK(mtcp, stream, cur_ts, 
					tcph, seq, ack_seq, window, payloadlen);
			//toe_prepare_host_buffer_enqueue(toe_eg, stream, qid);
			
			printf("%s-%d:inc_thresh:%d, stream->sndvar->increment_wnd:%u\n",__func__,__LINE__,inc_thresh, stream->sndvar->increment_wnd);
			if (stream->sndvar->increment_wnd >= inc_thresh) {
...
uint16_t toe_rx_pkt_burst(void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t nb_pkts)
{
	...
	if (ret > 0) {
		...
		if (TOE_LOG_ON) {
			...
			if (log_count < LOG_NUM) {
					timestamp[log_count] = (rte_rdtsc()*1000000)/rte_get_tsc_hz();
					sprintf(timestamp_str[log_count], "send_rx-pkt-burst"); 
					log_count ++;
			}
			else {
				timestamp[log_count] = (rte_rdtsc()*1000000)/rte_get_tsc_hz();                                

...
//		printf("%s-%d:cq_msg->data_len:%u, cq_msg->rq_head:%d, data_rq->pre_head:%d,data_rq->real_tail:%d,data_rq->enq_tail:%d, *data_rq->tail:%d\n",__func__,__LINE__,cq_msg->data_len,cq_msg->rq_head,data_rq->pre_head, data_rq->real_tail,data_rq->enq_tail,*data_rq->tail);
		printf("%s:%u,%d,%d,%d,%d,%d\n",__func__,cq_msg->data_len,cq_msg->rq_head,data_rq->pre_head, data_rq->real_tail,data_rq->enq_tail,*data_rq->tail);
		cq_info->tail = (cq_info->tail + 1) % cq_info->cq_size;
		if (cq_info->tail == 0)
			cq_info->cq_compl = cq_info->cq_compl ^ 1;
	}
...
static void toe_tx_data_dma_process(struct toe_sync_dma_job *sjob, struct toe_engine *toe_eg)
{
	...
	printf("%s-%d:,sjob->meb_num:%d, rq_info->tail:%d, rq_info->real_tail:%d,rq_info->enq_tail:%d,rq_info->pre_head:%d,rq_info->head:%d\n", __func__,__LINE__,sjob->meb_num,*rq_info->tail,rq_info->real_tail,rq_info->enq_tail,rq_info->pre_head,rq_info->head);
	printf("%s-%d:loop_count:%llu,now:%llu \n",__func__,__LINE__,loop_count,(rte_rdtsc()*1000000)/rte_get_tsc_hz());
	rte_mempool_put(toe_eg->t_dma->jpool, sjob);
	return;
}
...
int toe_tx_data_dma_enqueue(struct toe_engine *toe_eg, int idx)
{
	...
	printf("!! %s-%d:doorbell:%u, loop_count:%llu,now:%lu\n",__func__,__LINE__,*data_rq->tail, loop_count,(rte_rdtsc()*1000000)/rte_get_tsc_hz());
	rq_phy_base_addr = toe_addr32_to_addr64(data_rq->rbc->queue_desc_h, data_rq->rbc->queue_desc_lo);

...
uint8_t *toe_get_wptr_datapkt(struct mtcp_thread_context *ctx, uint16_t len, void *st, int data_posit)
{
	...
	printf("%s-%d:mbuf:%p,mbuf->data_len:%d,stream->sndvar->tcp_data_ring.mbuf_data_len[%d]:%u",__func__,__LINE__,mbuf,mbuf->data_len,data_posit, stream->sndvar->tcp_data_ring.mbuf_data_len[data_posit]);

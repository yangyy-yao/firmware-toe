...
_Noreturn static void * toe_thread_sys_ctrl_loop(void * reg)
{
	...
	while (true) {
		...
		if (!toe_dev->reset_done && toe_eg->t_dev->reset) {
		//	toe_reset_process(toe_eg);
			//ret = rte_ring_mc_dequeue_burst(rxq->rxq, (void **)pkts, nb_pkts, NULL); rest process 里处理
			return ret;
		}
		
		toe_sys_ctl_rq_dma_enqueue(toe_eg);
		toe_sys_ctl_cq_dma_enqueue(toe_eg);
		toe_sys_ctrl_dma_dequeue(toe_eg);
	}
}
...
static int toe_sys_ctrl_ring_init(struct toe_device *toe_dev)
{
	...
	ctl_vring = rte_calloc(NULL, 1, sizeof(struct toe_sys_ctl_vring), RTE_CACHE_LINE_SIZE);
	if (ctl_vring == NULL)
		goto fail;
...
static int toe_data_ring_init(struct toe_device *toe_dev)
{
	...
	for(i = 0; i < num; i ++) {
		...
		data_rx_vring[i]->rq_info.rbc = toe_rq_bar_get(toe_eg->bar, toe_dev->ctrl_queues + i + 1);
		...
		data_rx_vring[i]->cq_info.cbc = toe_cq_bar_get(toe_eg->bar, toe_dev->ctrl_queues + i + 1);
		...
		data_tx_vring[i]->rq_info.rbc = toe_rq_bar_get(toe_eg->bar, toe_dev->ctrl_queues + num + i + 1);
		...
		data_tx_vring[i]->cq_info.cbc = toe_cq_bar_get(toe_eg->bar, toe_dev->ctrl_queues + num + i + 1);
...
int toe_ctrl_thread_init(struct toe_device *toe_dev)
{
	...
	ret = pthread_create(&ctrl_thread, NULL, toe_thread_sys_ctrl_loop, (void *)toe_dev);
	if (ret){
	...
	if (lcoreid < 0)
		return ret;
	snprintf(thread_name, sizeof(thread_name), "toe_ctrl-c%02d", lcoreid);
	CPU_ZERO(&mask);

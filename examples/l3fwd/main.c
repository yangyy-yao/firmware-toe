...
#define RTE_LOGTYPE_GRO         RTE_LOGTYPE_USER1
...
struct lcore_conf l3_lcore_conf[RTE_MAX_LCORE];
...
static struct l3fwd_lkp_mode l3fwd_lpm_lkp = {
	...
	.cb_parse_ptype		= toe_cb_parse_ptype,
	.main_loop              = toe_main_loop,
...
init_lcore_rx_queues(void)
{
	...
	for (i = 0; i < nb_lcore_params; ++i) {
		lcore = lcore_params[i].lcore_id;
		nb_rx_queue = l3_lcore_conf[lcore].n_rx_queue;
		if (nb_rx_queue >= MAX_RX_QUEUE_PER_LCORE) {
			printf("error: too many queues (%u) for lcore: %u\n",
				(unsigned)nb_rx_queue + 1, (unsigned)lcore);
			return -1;
		} else {
			l3_lcore_conf[lcore].rx_queue_list[nb_rx_queue].port_id =
				lcore_params[i].port_id;
			l3_lcore_conf[lcore].rx_queue_list[nb_rx_queue].queue_id =
				lcore_params[i].queue_id;
			l3_lcore_conf[lcore].n_rx_queue++;
		}
	}
	return 0;
}
...
port_flow_complain(struct rte_flow_error *error)
{
	...
	printf("%s-%d:Caught error type %d (%s): %s%s: %s\n", __func__, __LINE__, error->type, errstr, error->cause ? (snprintf(buf, sizeof(buf), "cause: %p, ", error->cause), buf) : "", error->message ? error->message : "(no stated reason)", rte_strerror(err));
	return -err;
}
...
eth_flow_generate(int port_id, int max_queues)
{
	...
	int i;
	uint8_t mac[6] = {0xbe,0x18,0xb4,0x13,0x3c,0x5c};
...
struct rte_flow *
generate_tcp_flow(uint16_t port_id, uint16_t rx_q, int max_queue)
{
	struct rte_flow_attr attr;
	struct rte_flow_item pattern[2];
...
void toe_gro_init(int lcore_id)
{
    struct rte_gro_param param;
    //uint8_t lcore_id = rte_lcore_id();
    
	param.gro_types = RTE_GRO_TCP_IPV4;
	param.max_flow_num = GRO_MAX_FLUSH_CYCLES;
	param.max_item_per_flow = MAX_PKT_BURST;
    param.socket_id = rte_lcore_to_socket_id(lcore_id);
    
    toe_gro_ctx[lcore_id] = rte_gro_ctx_create(&param);
    RTE_LOG(INFO, GRO, "Create GRO CTX On Lcore.%u : %p\n", lcore_id, toe_gro_ctx[lcore_id]);
}
...
main(int argc, char **argv)
{
	...
	void* tcpstack_ctx;
	struct rte_flow *ethflow[RTE_MAX_ETHPORTS] = {NULL};
	struct rte_flow *tcpflow[RTE_MAX_ETHPORTS][RTE_MAX_LCORE] = {NULL};
	...
	RTE_ETH_FOREACH_DEV(portid) {
        ...
        printf("rte_eth_dev_adjust_nb_rx_tx_desc nb_rxd=%u nb_txd=%u \n", nb_rxd, nb_txd);
		...
		for (lcore_id = 0; lcore_id < RTE_MAX_LCORE; lcore_id++) {
			...
			printf("txq : lcore_id%u,queueid%d,socketid%d ", lcore_id, queueid, socketid);
			fflush(stdout);
	...
	for (lcore_id = 0; lcore_id < RTE_MAX_LCORE; lcore_id++) {
		if (rte_lcore_is_enabled(lcore_id) == 0)
			continue;
#ifdef toe_gro
		toe_gro_init(lcore_id);
#endif
		qconf = &l3_lcore_conf[lcore_id];
		printf("\nInitializing rx queues on lcore %u ... ", lcore_id );
		fflush(stdout);
		/* init RX queues */
//		tcpstack_ctx = InitializeMTCPManagerPerCore(lcore_id);
//		if (!tcpstack_ctx) {
//				rte_exit(EXIT_FAILURE,
//					"tcpstack_ctx: alloc failed!\n");
//		}
		...
		for(queue = 0; queue < qconf->n_rx_queue; ++queue) {
			...
			printf("rxq : portid%d,queueid%d,socketid%d \n", portid, queueid, socketid);
			fflush(stdout);
            ...
            printf("Re-adjust  nb_rxd=%u nb_txd=%u \n", nb_rxd, nb_txd);
...
/*
				if (portid == 0) {
					nb_rx_queue = get_port_n_rx_queues(portid);
					if (!queueid) {
						ethflow[portid] = eth_flow_generate(portid, nb_rx_queue);
						if (ethflow[portid] == NULL)
							rte_exit(EXIT_FAILURE, "eth_flow_generate failed:port=%d, core=%d\n",
														portid, lcore_id);
					}
					
					tcpflow[portid][lcore_id] = generate_tcp_flow(portid, queueid, nb_rx_queue);
					if (tcpflow[portid][lcore_id] == NULL) {
						rte_exit(EXIT_FAILURE, "tcp_flow_generate failed:port=%d, core=%d\n",
														portid, lcore_id);
					}
				}
				*/
...
*/
	...
	RTE_ETH_FOREACH_DEV(portid) {
		...
		/*
		if (ethflow[portid])
			flow_destroy(portid, ethflow[portid]);
		for (lcore_id = 0; lcore_id < RTE_MAX_LCORE; lcore_id++) {
			if (rte_lcore_is_enabled(lcore_id) == 0)
				continue;
			if (tcpflow[portid][lcore_id])
				flow_destroy(portid, tcpflow[portid][lcore_id]);
		}*/
		printf(" Done\n");
	}
	printf("Bye...\n");
